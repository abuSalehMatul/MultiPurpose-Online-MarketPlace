        ...
        <fieldset>
            ...
            <span v-if="is_show" class="no-record-span">{{no_record}}</span>
                <div class="wt-formoptions">
                    ...
                    <div class="wt-radioholder" v-bind:style='{"display" : (isActive? "block" : "none" )}'>
                        <span class="wt-radio" v-for="(filter, index) in filters" :key="index">
                            <input :id="filter.value" type="radio" name="searchtype" :value="filter.value" v-model="types" v-on:change="getSearchableData(types), emptyField(types)">
                            <label :for="filter.value">{{filter.title}}</label>
                        </span>
                    </div>
                    <a href="#" class="wt-searchbtn" v-on:click.prevent="submitSearchForm"><i class="lnr lnr-magnifier"></i><span>Search Now</span></a>
                </div>
            </div>
...
<script>
 export default{
    props: ['widget_type', 'no_record_message', 'placeholder', 'freelancer_placeholder', 'employer_placeholder', 'job_placeholder'],
        data(){
            return {
                filters:[],
                isActive: false,
        ...
        methods: {
            ...
            getSearchableData: function(type, newQuery){
                if (type) {
                    jQuery("input[name=searchtype][value=" + type + "]").attr('checked', 'checked');
                }
                this.types = type;
