    ...
    <div>
        ...
        <ul id="accordion" class="wt-historycontentcol">
            <li class="wt-historycolhead">
                <h3><span>{{ trans('lang.date') }}</span><span>{{ trans('lang.msg') }}</span><span>{{ trans('lang.attachment') }}</span></h3>
            </li>
            <span  v-for="(message, index) in messages" :key="index">
                <li class="collapsed" data-toggle="collapse" :data-target="'#collapseone-'+message.id">
                    <div class="wt-dateandmsg">
                        <span><img :src="message.user_image" alt="img description">{{message.created_at}}</span>
                        <span>{{message.excerpt}}</span>
                    </div>
                    <div class="wt-rightarea wt-msgbtns">
                        <a href="javascript:void(0);" class="wt-btn wt-msgbtn"><i class="lnr lnr-chevron-up"></i>{{ trans('lang.msg') }}</a>
                        <a :href="attahcments_url+message.id" class="wt-btn wt-attachmentbtn" v-if="message.attachments == 1"><i class="lnr lnr-download"></i>{{ trans('lang.attachment') }}</a>
                    </div>
                </li>
                <li class="wt-historydescription collapse" :id="'collapseone-'+message.id" data-parent="#accordion">
        ...
        <form class="wt-formtheme wt-userform" id="send_message_form" @submit.prevent="sendMessage()">
            <fieldset>
                ...
                <div class="form-group wt-btnarea">
                    <input type="submit" class="wt-btn" value="Send Now">
                </div>
            </fieldset>
        </form>
    </div>
...
<script>
 export default{
    props: ['id', 'recipent_id', 'upload_tmp_url', 'placeholder'],
        data(){
            return {
                isShow: false,
                proposal_id: this.id,
        ...
        methods: {
            ...
            sendMessage(){
                ...
                this.isShow = true;
                var self = this;
                axios.post(APP_URL + '/proposal/send-message',form_data)
                    .then(function (response) {
                        self.isShow = false;
                        if(response.data.type == 'success') {
                            jQuery('.wt-attachfile').find('li').remove();
                            jQuery('.wt-attachfile').find('input[type=hidden]').remove();
            ...
            getMessages(){
                let self = this;
                axios.post(APP_URL + '/proposal/get-private-messages',{
                    id: self.proposal_id,
