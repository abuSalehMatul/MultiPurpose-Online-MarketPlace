<template>
    <div :server_error_message="server_errors">
        <div class="wt-tabscontenttitle wt-addnew">
            ...
            <a href="javascript:void(0);" @click="addExperience" class="add-experience-btn">{{ trans('lang.add_experience') }}</a>
        </div>
        <ul class="wt-experienceaccordion accordion" id="experience-list">
            <span v-if="stored_experiences" class="experience-inner-list">
                 <li v-for="(stored_experience, index) in stored_experiences" :key="index" class="experience-element">
                     <div class="wt-accordioninnertitle">
                        <span :id="'experienceaccordion-'+index+''" data-toggle="collapse" :data-target="'#experienceaccordioninner-'+index+''">{{stored_experience.job_title}} ({{stored_experience.start_date}} - {{stored_experience.end_date}})</span>
                        <div class="wt-rightarea">
                            <a href="javascript:void(0);" class="wt-addinfo wt-skillsaddinfo" :id="'experienceaccordion-'+index+''" data-toggle="collapse" :data-target="'#experienceaccordioninner-'+index+''" aria-expanded="true"><i class="lnr lnr-pencil"></i></a>
                            <a href="javascript:void(0);" class="wt-deleteinfo" @click="removeStoredExperience(index)"><i class="lnr lnr-trash"></i></a>
                        </div>
                    </div>
                    <div class="wt-collapseexp collapse hide" :id="'experienceaccordioninner-'+index+''" :aria-labelledby="'experienceaccordion-'+index+''" data-parent="#accordion">
                        <fieldset>
                            ...
                            <div class="form-group">
                                <textarea :value="stored_experience.description" v-bind:name="'experience['+[index]+'][description]'" class="form-control" :placeholder="ph_job_desc"></textarea>
                            </div>
                            <div class="form-group">
            ...
            <span class="experience-inner-list">
                <li v-for="(experience, index) in experiences" :key="index" ref="experiencelistelement" class="experience-inner-list-item">
                    <div class="wt-accordioninnertitle">
                        <span :id="'experienceaccordion-'+experience.count+''" data-toggle="collapse" :data-parent="'#experienceaccordioninner-'+experience.count+''">
                            {{experience.job_title}} ( {{experience.start_date}} - {{experience.end_date}} )
                        </span>
                        <div class="wt-rightarea">
                            <a href="javascript:void(0);" class="wt-addinfo wt-skillsaddinfo" :id="'experienceaccordion-'+experience.count+''" data-toggle="collapse" :data-target="'#experienceaccordioninner-'+experience.count+''" aria-expanded="true"><i class="lnr lnr-pencil"></i></a>
                            <a href="javascript:void(0);" class="wt-deleteinfo delete-experience"><i class="lnr lnr-trash"></i></a>
                        </div>
                    </div>
                    <div class="wt-collapseexp collapse hide" :id="'experienceaccordioninner-'+experience.count+''" :aria-labelledby="'experienceaccordion-'+experience.count+''" data-parent="#accordion">
                        <fieldset>
                            <div class="form-group form-group-half">
                            ...
                            <div class="form-group">
                                <textarea v-bind:name="'experience['+[experience.count]+'][description]'" class="form-control" :placeholder="ph_job_desc"></textarea>
                            </div>
                            <div class="form-group">
                                <span>{{ trans('lang.date_note') }}</span>
                            </div>
...
export default{
    components: {DatePick},
    props: ['widget_title', 'server_error_message', 'server_errors', 'ph_job_title', 'ph_company_title', 'ph_job_desc'],
        data(){
            return {
        ...
        methods: {
            getExperiences(){
                let self = this;
                axios.get(APP_URL + '/freelancer/get-freelancer-experiences')
                .then(function (response) {
                    self.stored_experiences = response.data.experiences;
                });
            },
            addExperience: function () {
                var expereience_list_count = jQuery('.add-experience-btn').parents('.wt-tabsinfo').find('ul#experience-list span.experience-inner-list li').length;
                ...
                this.experiences.push(Vue.util.extend({}, this.experience, this.experience.count++))
            },
