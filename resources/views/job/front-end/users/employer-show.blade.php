                            ...
                            <div class="wt-proposalsr wt-proposalsrvtwo">
                                <div class="wt-widgetcontent wt-companysinfo">
                                    ...
                                    <div class="wt-title">
                                        @if ($user->user_verified === 1)
                                            <a href="{{{ url('Job/profile/'.$user->slug) }}}"><i class="fa fa-check-circle"></i> {{ trans('lang.verified_company') }}</a>
                                        @endif
                                        <h2>{{{ $user_name }}}</h2>
                                    </div>
                                </div>
                                <div class="tg-authorcodescan">
                                    <figure class="tg-qrcodeimg">
                                        {!! QrCode::size(100)->generate(Request::url('Job/profile/'.$user->slug)); !!}
                                    </figure>
                                    ...
                                    @if ($followers->count() > 0)
                                        <ul>
                                            @foreach ($followers as $follower)
                                                @php
                                                    $profile = \App\JobModel\JobProfile::all()->where('user_id', $follower->follower)->first();
                                                    $role_id = Helper::getRoleByUserID($follower->follower);
                                                @endphp
                                                @if (Helper::getRoleName($role_id) !== 'admin' && $follower->follower <> $user->id)
                                                    <li>
                                                        <a href="{{{url('Job/profile/'.$profile->user->slug)}}}">
                                            ...
                                            <div class="wt-viewjobholder">
                                                <ul>
                                                    ...
                                                    @if (!empty($job->location->title))
                                                        <li><span><img src="{{{asset(App\JobModel\JobHelper::getLocationFlag($job->location->flag))}}}" alt="{{{ trans('lang.location') }}}"> {{{ $job->location->title }}}</span></li>
                                                    @endif
                                                    ...
                                                    @endif
                                                    <li class="wt-btnarea"><a href="{{url('Job/job/'.$job->slug)}}" class="wt-btn">{{{ trans('lang.view_job') }}}</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
