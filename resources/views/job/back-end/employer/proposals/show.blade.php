                    ...
                    <div class="wt-dashboardboxcontent wt-jobdetailsholder">
                                ...
                                <div class="wt-userlistinghold wt-featured wt-userlistingvtwo">
                                    ...
                                    <div class="wt-userlistingcontent">
                                            ...
                                            @if (!empty($employer_name) || !empty($job->title) )
                                                <div class="wt-title">
                                                    @if (!empty($employer_name))
                                                        <a href="{{{ url('Job/profile/'.$job->employer->slug) }}}">@if ($verified_user === 1)<i class="fa fa-check-circle"></i>@endif&nbsp;{{{ $employer_name }}}</a>
                                                    @endif
                                                    @if (!empty($job->title))
                                                        <h2>{{{ $job->title }}}</h2>
                                                    @endif
                                            ...
                                            @if (!empty($job->price) ||
                                                ...
                                                <ul class="wt-saveitem-breadcrumb wt-userlisting-breadcrumb">
                                                    ...
                                                    @if (!empty($job->location->title))
                                                        <li><span><img src="{{{asset(App\JobModel\JobHelper::getLocationFlag($job->location->flag))}}}" alt="{{ trans('lang.img') }}"> {{{ $job->location->title }}}</span></li>
                                                    @endif
                                        ...
                                        <div class="wt-rightarea">
                                            ...
                                            @else
                                                <h4>{{ trans('lang.no_freelancers') }}</h4>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="wt-userlistinghold wt-featured wt-proposalitem">
                                        ...
                                        <div class="wt-proposaldetails">
                                            @if (!empty($freelancer_name))
                                                <div class="wt-contenthead">
                                                    <div class="wt-title">
                                                        <a href="{{ url('Job/profile/'.$user_slug) }}">{{{ $freelancer_name }}}</a>
                                                    </div>
                                                </div>
                                            @endif
                                        ...
                                        <div class="wt-rightarea wt-titlewithsearch">
                                                    ...
                                                    <fieldset>
                                                        <div class="form-group">
                                                            <span class="wt-select">
                                                                {!! Form::select('status', $project_status, $job->status, array('id' =>'job_status', 'data-placeholder' => trans('lang.select_status'), '@change' => 'jobStatus('.$job->id.', '.$accepted_proposal->id.')')) !!}
                                                            </span>
                                                            <a href="javascrip:void(0);" class="wt-searchgbtn" @click.prevent='jobStatus({{$job->id}})'><i class="fa fa-check"></i></a>
                                                        </div>
                                                    </fieldset>
                                                </form>
                                            @endif
                                            ...
                                            <div class="wt-hireduserstatus">
                                                ...
                                                {!! Form::open(['url' => url('Job/proposal/download-attachments'), 'class' =>'post-job-form wt-haslayout', 'id' => 'download-attachments-form-'.$accepted_proposal->freelancer_id]) !!}
                        ...
                        <div class="wt-projecthistory">
                            ...
                            <div class="wt-historycontent la-jobdetails-holder">
                                <private-message :placeholder="'{{ trans('lang.ph_job_dtl') }}'" :upload_tmp_url="'{{url('Job/proposal/upload-temp-image')}}'" :id="'{{$accepted_proposal->id}}'" :recipent_id="'{{$accepted_proposal->freelancer_id}}'"></private-message>
                            </div>
                        </div>
                    </div>
                </div>
