                                ...
                                <div class="wt-managejobcontent wt-verticalscrollbar mCustomScrollbar _mCS_1">
                                    @foreach ($ongoing_jobs as $job)
                                        @php
                                            ...
                                            $duration  =  \App\JobModel\JobHelper::getJobDurationList($job->duration);
                                            $user_name = $job->employer->first_name.' '.$job->employer->last_name;
                                            $accepted_proposal = \App\JobModel\JobJob::find($job->id)->proposals()->where('status', 'hired')->first();
                                            $freelancer_name = \App\JobModel\JobHelper::getUserName($accepted_proposal->freelancer_id);
                                            ...
                                            <div class="wt-userlistingcontent">
                                                <div class="wt-contenthead">
                                                    @if (!empty($user_name) || !empty($job->title) )
                                                        <div class="wt-title">
                                                            @if (!empty($user_name))
                                                                <a href="{{{ url('Job/profile/'.$job->employer->slug) }}}">@if($verified_user === 1)<i class="fa fa-check-circle"></i>@endif&nbsp;{{{ $user_name }}}</a>
                                                            @endif
                                                            @if (!empty($job->title))
                                                                <h2>{{{ $job->title }}}</h2>
                                                            @endif
                                                    ...
                                                    @if (!empty($job->price) ||
                                                        ...
                                                        <ul class="wt-saveitem-breadcrumb wt-userlisting-breadcrumb">
                                                            ...
                                                            @if (!empty($job->location->title))
                                                                <li><span><img src="{{{asset(App\JobModel\JobHelper::getLocationFlag($job->location->flag))}}}" alt="{{{ trans('lang.locations') }}}"> {{{ $job->location->title }}}</span></li>
                                                            @endif
