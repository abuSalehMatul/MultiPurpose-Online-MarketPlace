									...
									<div class="wt-managejobcontent">
										@foreach ($job_details as $job)
											@php
												...
												$proposals = \App\JobModel\JobProposal::where('job_id', $job->id)->where('status', '!=', 'cancelled')->get();
												$employer_img = \App\JobModel\JobProfile::select('avater')->where('user_id', $job->employer->id)->first();
												...
												<div class="wt-userlistingcontent">
													...
													<div class="wt-rightarea">
														<div class="wt-btnarea">
															<a href="{{{ url('Job/job/'.$job->slug) }}}" class="wt-btn">{{ trans('lang.view_detail') }}</a>
															<a href="{{{ url('Job/job/edit-job/'.$job->slug) }}}" class="wt-btn">{{ trans('lang.edit_job') }}</a>
															@if ($proposals->count() > 0)
																<a href="{{{ url('Job/employer/dashboard/job/'.$job->slug.'/proposals') }}}" class="wt-btn">{{ trans('lang.view_proposals') }}</a>
															@endif
														</div>
