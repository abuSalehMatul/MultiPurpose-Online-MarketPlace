                                ...
                                @if (!empty($saved_jobs))
                                    <div class="wt-dashboradsaveitem">
                                        @foreach($saved_jobs as $job_id)
                                            @php
                                                $job = \App\JobModel\JobJob::where('id', $job_id)->first();
                                                ...
                                                <div class="wt-userlistingcontent">
                                                    <div class="wt-contenthead wt-dashboardsavehead">
                                                        @if (!empty($user_name) || !empty($job->title) )
                                                            <div class="wt-title">
                                                                @if (!empty($user_name))
                                                                <a href="{{{ url('Job/profile/'.$job->employer->slug) }}}">@if($verified_user === 1)
                                                                    <i class="fa fa-check-circle"></i>@endif&nbsp;{{{ $user_name }}}</a>
                                                                @endif
                                                                @if (!empty($job->title))
                                                                    <h2><a href="{{ url('Job/job/'.$job->slug) }}">{{{ $job->title }}}</a></h2>
                                                                @endif
                                                            </div>
                                                        @endif
                                                        @if (!empty($job->price) ||
                                                ...
                                                <div class="wt-userlistinghold wt-userlistingsingle">
                                                    ...
                                                    <div class="wt-userlistingcontent">
                                                        <div class="wt-contenthead wt-followcomhead">
                                                            <div class="wt-title">
                                                                <a href="{{{ url('Job/profile/'.$emp->slug) }}}">
                                                                    @if($verified_user === 1)
                                                                        <i class="fa fa-check-circle"></i> {{ trans('lang.verified_company') }}</a>
                                                                    @endif
                                                                <h3><a href="{{ url('Job/profile/'.$emp->slug) }}">{{{ $user_name }}}</a></h3>
                                                            </div>
                                                            <ul class="wt-followcompomy-breadcrumb wt-userlisting-breadcrumb">
                                                                <li><a href="{{{ url('Job/profile/'.$emp->slug) }}}"> {{ trans('lang.open_jobs') }}  </a></li>
                                                                <li><a href="{{{ url('Job/profile/'.$emp->slug) }}}"> {{ trans('lang.full_profile') }}</a></li>
                                ...
                                <div class="wt-likedfreelancers wt-haslayout">
                                    @foreach ($saved_freelancers as $key => $freelancer)
                                        @php
                                            ...
                                            $proposal = \App\JobModel\JobProposal::select('job_id')->where('freelancer_id', $user->id)->pluck('job_id')->first();
                                            $employer_id = !empty($proposal) ? \App\JobModel\JobJob::select('user_id')->where('id', $proposal)->pluck('user_id')->first() : '';
                                            $job = !empty($employer_id) ? \App\JobModel\JobJob::where('user_id', $employer_id)->first() : '';
                                                ...
                                                <div class="wt-contenthead">
                                                    <div class="wt-title">
                                                        <a href="{{{ url('Job/profile/'.$user->slug) }}}">
                                                            ...
                                                            {{{ $user_name }}}</a>
                                                        <h2><a href="{{ url('Job/profile/'.$user->slug) }}">{{{ $profile->tagline }}}</a></h2>
                                                    </div>
