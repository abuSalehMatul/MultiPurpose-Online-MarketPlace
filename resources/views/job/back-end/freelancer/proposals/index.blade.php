                            ...
                            @if (!empty($proposals) && $proposals->count() > 0)
                                <div class="wt-managejobcontent">
                                    @foreach ($proposals as $proposal)
                                        @php
                                            $freelancer_proposal = \App\JobModel\JobProposal::find($proposal->id);
                                            ...
                                            $detail_link = $proposal->status == 'hired' ? url('Job/freelancer/job/'.$proposal->job->slug) : 'javascript:void(0);';
                                            ...
                                            <div class="wt-userlistingcontent wt-userlistingcontentvtwo">
                                                <div class="wt-contenthead">
                                                    @if (!empty($user_name) || !empty($proposal->job->title) )
                                                        <div class="wt-title">
                                                            @if (!empty($user_name))
                                                            <a href="{{{ url('Job/profile/'.$proposal->job->employer->slug) }}}">
                                                ...
                                                <div class="wt-rightarea la-pending-jobs">
                                                    <div class="wt-hireduserstatus">
                                                        ...
                                                        @if ( $proposal->status != 'pending' )
                                                            <a href="{{{ url('Job/freelancer/job/'.$proposal->job->slug) }}}" class="wt-btn">
                                                                {{$status_btn}}
                                                            </a>
                                                        @endif
                                                    </div>
