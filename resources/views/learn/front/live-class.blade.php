    ...
    <div class="container pad-80-20">
        <div class="row">
            ...
            <div class="col-sm-3 width-40">
                ...
                <ul id="nav-tabs-wrapper" class="nav nav-tabs nav-pills nav-stacked well do-nicescrol">
                    @foreach($topics as $k=>$data)
                        <li @if($k==0)class="active" id="active_id" @endif>
                            <a href="#vtab{{$data->id}}" data-embedecode="{{$data->link}}" data-embededtitle="{{$data->title}}" data-id="{{$data->id}}" class="stop-video" data-toggle="tab" >{{$data->title}}</a></li>
                    @endforeach
                </ul>
            </div>
                ...
                @else
                <div class="col-sm-12">
                    <h1 class="text-center">No Topics Found !!</h1>
                </div>
