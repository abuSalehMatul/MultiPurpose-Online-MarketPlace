...
@section('content')
  <div class="content-main-block  mrg-t-40">
    <div class="admin-create-btn-block">
      <a href="{{route('forumcategory.create')}}" class="btn btn-danger btn-md">Add Discussion Category</a>
      <!-- Delete Modal -->
        ...
        <div class="modal-dialog modal-sm">
          ...
          <div class="modal-content">
            ...
            <div class="modal-footer">
              {!! Form::open(['method' => 'POST', 'action' => 'ForumCategoryController@bulk_delete', 'id' => 'bulk_delete_form']) !!}
    ...
    <div class="content-block box-body">
      <table id="full_detail_table" class="table table-hover table-responsive">
        ...
        @if (isset($forumcategory))
          <tbody>
            @foreach ($forumcategory as $key => $item)
              <tr>
                <td>
                  <div class="inline">
                ...
                <td>{{$item->category == 'c' ? 'Coupon' : ($item->category =='d' ? 'Deal' : 'Discussion')}}</td>
                <td>{{$item->title}}</td>
                <td>{!!  substr(strip_tags($item->detail), 0, 30) !!}...</td>
                <td>{{$item->is_active == '1' ? 'Active' : 'Deactive'}}</td>
                <td>
                  <div class="admin-table-action-block">
                    <a href="{{route('forumcategory.edit', $item->id)}}" data-toggle="tooltip" data-original-title="Edit" class="btn-info btn-floating"><i class="material-icons">mode_edit</i></a>
                    <!-- Delete Modal -->
                    ...
                    <div id="{{$item->id}}deleteModal" class="delete-modal modal fade" role="dialog">
                      <div class="modal-dialog modal-sm">
                        ...
                        <div class="modal-content">
                          ...
                          <div class="modal-footer">
                            {!! Form::open(['method' => 'DELETE', 'action' => ['ForumCategoryController@destroy', $item->id]]) !!}
                                <button type="reset" class="btn btn-gray translate-y-3" data-dismiss="modal">No</button>
                                <button type="submit" class="btn btn-danger">Yes</button>
                            {!! Form::close() !!}
                          </div>
