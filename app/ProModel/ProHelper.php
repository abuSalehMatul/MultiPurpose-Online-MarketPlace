...
use App\User;
use App\ProModel\ProProSiteManagement;
use App\Badge;
...
class ProHelper extends Model
{
    ...
    {
        $output = "";
        $social_unserialize_array = ProSiteManagement::getMetaValue('socials');
        $social_list = ProHelper::getSocialData();
    ...
    {
        $parent_cat = ProLocation::select('title', 'id', 'parent')->where('parent', $parent_id)->get()->toArray();
    ...
    {
        return ProProposal::select('job_id')->latest()->where('freelancer_id', $user_id)->where('status', $status)->get();
    }
    ...
    public static function getEmployerJobs($user_id)
    {
        if (!empty($user_id)) {
            ...
            if ($user->getRoleNames()->first() === 'employer') {
                return ProJob::select('title', 'id')->where('user_id', $user_id)->get()->pluck('title', 'id');
            } else {
                return array();
            }
        } else {
    ...
    public static function updatePayouts()
    {
        ...
        $query = ProProposal::select('freelancer_id', DB::raw('sum(amount) earning'))->where('status', 'completed')
    ...
    {
        $home_page_settings = !empty(ProSiteManagement::getMetaValue('home_settings')) ? ProSiteManagement::getMetaValue('home_settings') : array();
    ...
    {
        $section_settings = !empty(ProSiteManagement::getMetaValue('section_settings')) ? ProSiteManagement::getMetaValue('section_settings') : array();
    ...
    public static function getAppSection($type)
    {
        ...
        $application_desc = ProSiteManagement::where('meta_key', 'app_desc')->select('meta_value')->pluck('meta_value')->first();
        $app_desc = !empty($application_desc) ? $application_desc : '';
        $application_android_link = ProSiteManagement::where('meta_key', 'app_android_link')->select('meta_value')->pluck('meta_value')->first();
        $app_android_link = !empty($application_android_link) ? $application_android_link : '#';
        $application_ios_link = ProSiteManagement::where('meta_key', 'app_ios_link')->select('meta_value')->pluck('meta_value')->first();
    ...
    {
        $styling = ProSiteManagement::getMetaValue('styling');
        dd($styling);
        if (!empty($styling)) {
            ob_start(); ?>
        <style>
