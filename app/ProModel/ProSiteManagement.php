 ...
 *
 */
class ProSiteManagement extends Model
{
    /**
     * Add Fillables
     *
    ...
    {
        if (!empty($meta_key)) {
            $data = DB::table('pro_site_managements')->select('meta_value')->where('meta_key', $meta_key)->get()->first();
            if (!empty($data)) {
    ...
    public static function saveEmailSettings($email_data)
    {
        ...
        if (!empty($email_data)) {
            $old_path = ProHelper::PublicPath() . '/uploads/settings/temp';
            if (!file_exists($old_path)) {
                File::makeDirectory($old_path, 0755, true, true);
            }
            $new_path = ProHelper::PublicPath() . '/uploads/settings/email';
            ...
            $existing_data = ProSiteManagement::getMetaValue('email_data');
            if (!empty($existing_data)) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'email_data')->delete();
            }
            DB::table('pro_site_managements')->insert(
                [
    ...
    public static function saveSettings($settings)
    {
        ...
        if (!empty($settings)) {
            $old_path = ProHelper::PublicPath() . '/uploads/settings/temp';
            $new_path = ProHelper::PublicPath() . '/uploads/settings/general';
            ...
            $existing_data = ProSiteManagement::getMetaValue('settings');
            if (!empty($existing_data)) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'settings')->delete();
            }
            DB::table('pro_site_managements')->insert(
                [
    ...
    public static function saveIcons($request)
    {
        ...
        if (!empty($request['icons'])) {
            $icons = $request['icons'];
            $old_path = ProHelper::PublicPath() . '/uploads/settings/temp';
            $new_path = ProHelper::PublicPath() . '/uploads/settings/icon';
            ...
            $existing_data = ProSiteManagement::getMetaValue('icons');
            if (!empty($existing_data)) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'icons')->delete();
            }
            DB::table('pro_site_managements')->insert(
                [
    ...
    public static function saveThemeStylingSettings($request)
    {
        if (!empty($request['styling'])) {
            ...
            $existing_data = ProSiteManagement::getMetaValue('styling');
            if (!empty($existing_data)) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'styling')->delete();
            }
            DB::table('pro_site_managements')->insert(
                [
    ...
    public static function saveFooterSettings($footer_settings)
    {
        if (!empty($footer_settings)) {
            $old_path = ProHelper::PublicPath() . '/uploads/settings/temp';
            $new_path = ProHelper::PublicPath() . '/uploads/settings/footer';
            ...
            $existing_data = ProSiteManagement::getMetaValue('footer_settings');
            if (!empty($existing_data)) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'footer_settings')->delete();
            }
            DB::table('pro_site_managements')->insert(
                [
    ...
    public static function saveSocialSettings($request)
    {
        ...
        if (!empty($socials)) {
            ...
            $existing_social = ProSiteManagement::getMetaValue('socials');
            if (!empty($existing_social)) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'socials')->delete();
            }
            DB::table('pro_site_managements')->insert(
                [
    ...
    public static function saveSearchMenu($request)
    {
        ...
        if (!empty($menu)) {
            ...
            $existing_menu_item = ProSiteManagement::getMetaValue('search_menu');
            if (!empty($existing_menu_item)) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'search_menu')->delete();
            }
            DB::table('pro_site_managements')->insert(
                [
    ...
    public function saveCommisionSettings($commision_settings)
    {
        ...
        if (!empty($commision_settings)) {
            ...
            $existing_data = ProSiteManagement::getMetaValue('commision');
            if (!empty($existing_data)) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'commision')->delete();
                ProHelper::changeEnv(
                    [
                        'PAYMENT_SYMBOL' => '',
                    ]
    ...
    public function savePaymentSettings($request)
    {
        ...
        if (!empty($payment_settings)) {
            $existing_payment_settings = ProSiteManagement::getMetaValue('payment_settings');
            if (!empty($existing_payment_settings)) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'payment_settings')->delete();
                ProHelper::changeEnv(
                    [
    ...
    public function saveStripeSettings($request)
    {
        ...
        if (!empty($payment_settings)) {
            $existing_payment_settings = ProSiteManagement::getMetaValue('stripe_settings');
            if (!empty($existing_payment_settings)) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'stripe_settings')->delete();
                ProHelper::changeEnv(
                    [
                        'STRIPE_KEY' => "",
                        'STRIPE_SECRET' => "",
    ...
    public static function saveHomeSettings($home_settings, $request)
    {
        if (!empty($home_settings) || !empty($request)) {
            $old_path = ProHelper::PublicPath() . '/uploads/settings/temp';
            $new_path = ProHelper::PublicPath() . '/uploads/settings/home';
            ...
            $existing_data = ProSiteManagement::getMetaValue('home_settings');
            if (!empty($existing_data)) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'home_settings')->delete();
            }
            DB::table('pro_site_managements')->insert(
                [
    ...
    public static function saveSectionSettings($section_settings, $request)
    {
        if (!empty($section_settings) || !empty($request)) {
            ...
            $existing_data = ProSiteManagement::getMetaValue('section_settings');
            if (!empty($existing_data)) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'section_settings')->delete();
            }
            DB::table('pro_site_managements')->insert(
                [
            ...
            if (!empty($request['app_desc'])) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'app_desc')->delete();
                DB::table('pro_site_managements')->insert(
                    [
            ...
            if (!empty($request['app_android_link'])) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'app_android_link')->delete();
                DB::table('pro_site_managements')->insert(
                    [
            ...
            if (!empty($request['app_ios_link'])) {
                DB::table('pro_site_managements')->where('meta_key', '=', 'app_ios_link')->delete();
                DB::table('pro_site_managements')->insert(
                    [
    ...
    public static function saveRegistrationSettings($request)
    {
        if (!empty($request)) {
            if (!empty($request['registration'][0])) {
                ...
                $existing_data = ProSiteManagement::getMetaValue('reg_form_settings');
                if (!empty($existing_data)) {
                    DB::table('pro_site_managements')->where('meta_key', '=', 'reg_form_settings')->delete();
                }
                DB::table('pro_site_managements')->insert(
                    [
