...
class ProJob extends Model
{
    ...
    {
        return $this->belongsToMany('App\ProModel\ProSkill');
    }
    ...
    {
        return $this->hasMany('App\ProModel\ProProposal');
    }
    ...
    public function setSlugAttribute($value)
    {
        if (!empty($value)) {
            $temp = str_slug($value, '-');
            if (!ProJob::all()->where('slug', $temp)->isEmpty()) {
                $i = 1;
                $new_slug = $temp . '-' . $i;
                while (!ProJob::all()->where('slug', $new_slug)->isEmpty()) {
                    $i++;
                    $new_slug = $temp . '-' . $i;
                }
                $temp = $new_slug;
    ...
    public static function deleteRecord($id)
    {
        ...
        if (!empty($job->proposals)) {
            foreach ($job->proposals as $key => $proposal) {
                $proposal = ProProposal::find($proposal->id);
                $proposal->delete();
            }
        }
        $job->skills()->detach();
