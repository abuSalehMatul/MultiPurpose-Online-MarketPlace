 ...
 *
 */
class ProLocation extends Model
{
    /**
    ...
    public function setSlugAttribute($value)
    {
        if (!empty($value)) {
            $temp = str_slug($value, '-');
            if (!ProLocation::all()->where('slug', $temp)->isEmpty()) {
                $i = 1;
                $new_slug = $temp . '-' . $i;
                while (!ProLocation::all()->where('slug', $new_slug)->isEmpty()) {
                    $i++;
                    $new_slug = $temp . '-' . $i;
                }
                $temp = $new_slug;
    ...
    {
        $query = DB::table('pro_locations')->select('*')->where('parent', '=', $parent_id)->get();
        $output = '';
    ...
    {
        $ancestors = DB::table('pro_locations')->select('*')->where('id', '=', $id)->get();
        while ($ancestors->last() && $ancestors->last()->parent !== null) {
            $parent = DB::table('pro_locations')->select('*')->where('id', '=', $ancestors->last()->parent)->get();
