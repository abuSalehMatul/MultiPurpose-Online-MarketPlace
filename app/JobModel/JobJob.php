...
class JobJob extends Model
{
    ...
    {
        return $this->belongsToMany('App\JobModel\JobSkill');
    }
    ...
    /**
     * Get the Jobposals associated with job.
     *
     * @return relation
     */
    public function proposals()
    {
        return $this->hasMany('App\JobModel\JobProposal');
    }
    ...
    public function setSlugAttribute($value)
    {
        if (!empty($value)) {
            $temp = str_slug($value, '-');
            if (!JobJob::all()->where('slug', $temp)->isEmpty()) {
                $i = 1;
                $new_slug = $temp . '-' . $i;
                while (!JobJob::all()->where('slug', $new_slug)->isEmpty()) {
                    $i++;
                    $new_slug = $temp . '-' . $i;
                }
                $temp = $new_slug;
    ...
    public static function deleteRecord($id)
    {
        ...
        if (!empty($job->proposals)) {
            foreach ($job->proposals as $key => $proposal) {
                $proposal = JobProposal::find($proposal->id);
                $proposal->delete();
            }
        }
        $job->skills()->detach();
