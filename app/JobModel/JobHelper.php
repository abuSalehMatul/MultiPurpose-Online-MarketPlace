...
class JobHelper extends Model
{
    ...
    public static function getSearchFilterList($key = "")
    {
        $list = array(
            '0' => array(
                'title' => trans('lang.search_filter_list.candidate'),
                'value' => 'candidate',
            ),
            '1' => array(
    ...
    {
        $parent_cat = JobLocation::select('title', 'id', 'parent')->where('parent', $parent_id)->get()->toArray();
    ...
    {
        return JobProposal::select('job_id')->latest()->where('freelancer_id', $user_id)->where('status', $status)->get();
    }
    ...
    public static function getEmployerJobs($user_id)
    {
        if (!empty($user_id)) {
            ...
            if ($user->getRoleNames()->first() === 'employer') {
                return JobJob::select('title', 'id')->where('user_id', $user_id)->get()->pluck('title', 'id');
            } else {
                return array();
            }
        } else {
    ...
    public static function updatePayouts()
    {
        ...
        $query = JobProposal::select('freelancer_id', DB::raw('sum(amount) earning'))->where('status', 'completed')
    ...
    {
        $home_page_settings = !empty(JobSiteManagement::getMetaValue('home_settings')) ? JobSiteManagement::getMetaValue('home_settings') : array();
    ...
    public static function getAppSection($type)
    {
        ...
        $application_desc = JobSiteManagement::where('meta_key', 'app_desc')->select('meta_value')->pluck('meta_value')->first();
        $app_desc = !empty($application_desc) ? $application_desc : '';
        $application_android_link = JobSiteManagement::where('meta_key', 'app_android_link')->select('meta_value')->pluck('meta_value')->first();
        $app_android_link = !empty($application_android_link) ? $application_android_link : '#';
        $application_ios_link = JobSiteManagement::where('meta_key', 'app_ios_link')->select('meta_value')->pluck('meta_value')->first();
    ...
    {
        $styling = JobSiteManagement::getMetaValue('styling');
        dd($styling);
        if (!empty($styling)) {
            ob_start(); ?>
        <style>
