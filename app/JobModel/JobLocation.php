...
class JobLocation extends Model
{
    ...
    public function setSlugAttribute($value)
    {
        if (!empty($value)) {
            $temp = str_slug($value, '-');
            if (!JobLocation::all()->where('slug', $temp)->isEmpty()) {
                $i = 1;
                $new_slug = $temp . '-' . $i;
                while (!JobLocation::all()->where('slug', $new_slug)->isEmpty()) {
                    $i++;
                    $new_slug = $temp . '-' . $i;
                }
                $temp = $new_slug;
    ...
    {
        $query = DB::table('job_locations')->select('*')->where('parent', '=', $parent_id)->get();
        $output = '';
    ...
    {
        $ancestors = DB::table('job_locations')->select('*')->where('id', '=', $id)->get();
        while ($ancestors->last() && $ancestors->last()->parent !== null) {
            $parent = DB::table('job_locations')->select('*')->where('id', '=', $ancestors->last()->parent)->get();
