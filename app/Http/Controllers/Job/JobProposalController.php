...
class JobProposalController extends Controller
{
     ...
     * @return void
     */
    public function __construct(JobProposal $proposal)
    {
    ...
    {
        if (!empty($job_slug)) {
            $job = JobJob::all()->where('slug', $job_slug)->first();
            if (!empty($job)) {
                $job_skills = $job->skills->pluck('id')->toArray();
                $check_skill_req = $this->proposal->getJobSkillRequirement($job_skills);
                $proposal_status = JobJob::find($job->id)->proposals()->where('status', 'hired')->first();
                ...
                $duration =  JobHelper::getJobDurationList($job->duration);
                $job_completion_time = JobHelper::getJobDurationList();
                $commision_amount = JobSiteManagement::getMetaValue('commision');
    ...
    public function uploadTempImage(Request $request)
    {
        if (!empty($request['file'])) {
            ...
            return JobHelper::uploadTempMultipleAttachments($attachments, $path);
        }
    }
    ...
    public function store(Request $request)
    {
        if (Auth::user()) {
            ...
            if (!empty($request)) {
                ...
                $package = DB::table('job_items')->where('subscriber', Auth::user()->id)->select('product_id')->first();
                            ...
                            if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                                if (!empty($job->employer->email)) {
                                    ...
                                    if (
                                        ...
                                        $email_params['freelancer'] = JobHelper::getUserName(Auth::user()->id);
                                        ...
                                        $email_params['duration'] = JobHelper::getJobDurationList($request['completion_time']);
    ...
    public function getJobProposals($slug)
    {
        if (!empty($slug)) {
            ...
            $proposals = JobJob::latest()->find($job->id)->proposals;
            $accepted_proposal = JobJob::find($job->id)->proposals()->where('hired', 1)->first();
            $duration = !empty($job->duration) ? JobHelper::getJobDurationList($job->duration) : '';
            $currency   = JobSiteManagement::getMetaValue('commision');
            $symbol = !empty($currency) && !empty($currency[0]['currency']) ? JobHelper::currencyList($currency[0]['currency']) : array();
            return View(
                'job.back-end.employer.proposals.index',
                compact(
                    'proposals',
                    'job',
                    'duration',
    ...
    public function show($slug)
    {
        ...
        $attachments = array();
        $job = JobJob::where('slug', $slug)->first();
        $accepted_proposal = JobJob::find($job->id)->proposals()->where('hired', 1)
            ->first();
        $freelancer_name = JobHelper::getUserName($accepted_proposal->freelancer_id);
        $completion_time = !empty($accepted_proposal->completion_time) ?
            JobHelper::getJobDurationList($accepted_proposal->completion_time) : '';
        ...
        $project_status = JobHelper::getProjectStatus();
        $duration = !empty($job->duration) ? JobHelper::getJobDurationList($job->duration) : '';
        ...
        $symbol = !empty($currency) && !empty($currency[0]['currency']) ? JobHelper::currencyList($currency[0]['currency']) : array();
        return view(
            'job.back-end.employer.proposals.show',
            compact(
                'job',
                'duration',
                'accepted_proposal',
