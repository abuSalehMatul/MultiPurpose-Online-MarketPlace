 ...
 *
 */
class JobPaypalController extends Controller
{
        ...
        if (Auth::user()) {
            ...
            if (Auth::user()->getRoleNames()[0] == "employer") {
                if ($product_type == 'project') {
                    return Redirect::to('Job/employer/jobs/hired');
                } else {
                    return Redirect::to('Job/dashboard/packages/employer');
                }
            } else if (Auth::user()->getRoleNames()[0] == "freelancer") {
                session()->forget('type');
                return Redirect::to('Job/dashboard/packages/freelancer');
            }
        } else {
            abort(404);
        }
    ...
    public function getExpressCheckoutSuccess(Request $request)
    {
        if (Auth::user()) {
            ...
            if (in_array(strtoupper($response['ACK']), ['SUCCESS', 'SUCCESSWITHWARNING'])) {
                ...
                return redirect('Job/paypal/redirect-url');
            } else {
                abort(404);
            }
        }
    ...
    protected function getCheckoutData($recurring, $success)
    {
        if (Auth::user()) {
            ...
            if ($recurring === true) {
                ...
                $data['return_url'] = url('Job/paypal/ec-checkout-success?mode=recurring');
                ...
                $data['return_url'] = url('Job/paypal/ec-checkout-success');
            }
    ...
    protected function createInvoice($cart, $status, $payment_detail)
    {
        ...
        collect(
            ...
            function ($product) use ($invoice) {
                ...
                if ($product_type == 'package') {
                    $item = DB::table('job_items')->select('id')->where('subscriber', $product['subscriber_id'])->first();
                    if (!empty($item)) {
        ...
        if (Auth::user()) {
            if ($product_type == 'package') {
                if (session()->has('product_id')) {
                    $package_item = \App\JobModel\JobItem::where('subscriber', Auth::user()->id)->first();
                    $id = session()->get('product_id');
                    $package = \App\JobModel\JobPackage::find($id);
                ...
                $proposal->save();
                $job = JobJob::find($proposal->job->id);
                $job->status = 'hired';
                $job->save();
        ...
        if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
            if ($product_type == 'project') {
                $id = session()->get('product_id');
                $proposal =  JobProposal::find($id);
                $job = \App\JobModel\JobJob::find($proposal->job->id);
                ...
                if (!empty($freelancer->email)) {
                    ...
                    if (!empty($template->id)) {
                        ...
                        $email_params['hired_project_link'] = url('Job/job/'.$job->slug);
                        $email_params['name'] = JobHelper::getUserName($freelancer->id);
                        $email_params['link'] = url('Job/profile/'.$freelancer->slug);
                        $email_params['employer_profile'] = url('Job/profile/'.$employer->slug);
                        $email_params['emp_name'] = JobHelper::getUserName($employer->id);
                ...
                $id = session()->get('product_id');
                $item = DB::table('job_items')->where('product_id', $id)->get()->toArray();
                $package =  JobPackage::where('id', $item[0]->product_id)->first();
                ...
                if ($role === 'employer') {
                    if (!empty($user->email)) {
                        ...
                        if (!empty($template->id)) {
                            $template_data = EmailTemplate::getEmailTemplateByID($template->id);
                            $email_params['employer'] = JobHelper::getUserName(Auth::user()->id);
                    ...
                    if (!empty(Auth::user()->email)) {
                        ...
                        if (!empty($template->id)) {
                            ...
                            $email_params['freelancer_profile'] = url('Job/profile/'.Auth::user()->slug);
                            $email_params['name'] = $package->title;
                            $email_params['price'] = $package->cost;
                            $email_params['expiry_date'] = !empty($expiry_date) ? Carbon::parse($expiry_date)->format('M d, Y') : '';
                            Mail::to(Auth::user()->email)
