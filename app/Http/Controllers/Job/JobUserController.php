...
class JobUserController extends Controller
{
     ...
     * @return void
     */
    public function __construct(User $user, JobProfile $profile)
    {
        $this->user = $user;
    ...
    {
        return view('job.back-end.settings.reset-password');
    }
                ...
                if ($request->confirm_password === $request->confirm_new_password) {
                    ...
                    if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                        ...
                        if (!empty($template->id)) {
                            ...
                            $email_params['name'] = JobHelper::getUserName($user_id);
    ...
    public function emailNotificationSettings()
    {
        ...
        return view('job.back-end.settings.email-notifications', compact('user_email'));
    }
    ...
    {
        return view('job.back-end.settings.delete-account');
    }
    ...
    public function destroy(Request $request)
    {
        ...
        if (Hash::check($request->old_password, $user->password)) {
            if (!empty($user_id)) {
                ...
                $user->delete();
                if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                    $delete_reason = JobHelper::getDeleteAccReason($request['delete_reason']);
                    $email_params = array();
        ...
        if (!empty($request['user_id'])) {
            ...
            if (!empty($user)) {
                ...
                DB::table('messages')->where('user_id', $request['user_id'])
                    ->orWhere('receiver_id', $request['user_id'])->delete();
                DB::table('job_items')->where('subscriber', $request['user_id'])
                    ->delete();
    ...
    public function getSavedItems(Request $request, $role = '')
    {
        if (Auth::user()) {
            ...
            if ($request->path() === 'employer/saved-items') {
                return view(
                    'job.back-end.employer.saved-items',
                    compact(
                        'profile',
                        'saved_jobs',
                        'saved_freelancers',
                ...
                return view(
                    'job.back-end.freelancer.saved-items',
                    compact(
                        'profile',
                        'saved_jobs',
                        'saved_freelancers',
    ...
    public function submitReview(Request $request)
    {
        ...
        if (Auth::user()) {
            ...
            if ($submit_review['type'] == "success") {
                ...
                // Send Email
                $freelancer = JobProposal::select('freelancer_id')->where('status', 'completed')->first();
                $user = User::find($freelancer->freelancer_id);
                $job = JobJob::find($request['job_id']);
                //send email
                if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                    $email_params = array();
                    ...
                    if (!empty($job_completed_template->id)) {
                        ...
                        $email_params['completed_project_link'] = url('Job/job/' . $job->slug);
                        $email_params['name'] = JobHelper::getUserName($freelancer->freelancer_id);
                ...
                if ($request['report_type'] == 'job-report'
                    ...
                    if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                        ...
                        if ($request['report_type'] == 'job-report') {
                            ...
                            if (!empty($report_project_template->id)) {
                                ...
                                $email_params['reported_by'] = JobHelper::getUserName(Auth::user()->id);
                            ...
                            if (!empty($report_employer_template->id)) {
                                ...
                                $email_params['reported_employer'] = JobHelper::getUserName($request['id']);
                                $email_params['link'] = url('Job/profile/' . $employer->slug);;
                                $email_params['report_by_link'] = url('Job/profile/' . $user->slug);
                                $email_params['reported_by'] = JobHelper::getUserName(Auth::user()->id);
                                $email_params['message'] = $request['description'];
                                Mail::to(config('mail.username'))
                            ...
                            if (!empty($report_freelancer_template->id)) {
                                ...
                                $email_params['reported_freelancer'] = JobHelper::getUserName($request['id']);
                    ...
                    if (!empty($freelancer_job_cancelled->id)) {
                        if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                            ...
                            $email_params['project_title'] = $job->title;
                            $email_params['cancelled_project_link'] = url('Job/job/' . $job->slug);
                            $email_params['name'] = JobHelper::getUserName($proposal->freelancer_id);
                            $email_params['link'] = url('Job/profile/' . $freelancer->slug);
                            $email_params['employer_profile'] = url('Job/profile/' . Auth::user()->slug);
                            $email_params['emp_name'] = JobHelper::getUserName(Auth::user()->id);
    ...
    public function sendPrivateMessage(Request $request)
    {
        if (Auth::user()) {
            ...
            if (!empty($request['description'])) {
                $user_id = Auth::user()->id;
                $json = array();
                $proposal = DB::table('job_proposals')->select('status')->where('id', $request['proposal_id'])->get()->first();
    ...
    {
        if (!empty($id)) {
            $package_options = JobHelper::getPackageOptions(Auth::user()->getRoleNames()[0]);
            ...
            $symbol = !empty($payout_settings) && !empty($currency[0]['currency']) ? JobHelper::currencyList($payout_settings[0]['currency']) : array();
            return view::make('job.back-end.package.checkout', compact('package', 'package_options', 'payment_gateway', 'symbol'));
        }
    }
        ...
        if (Auth::user()->getRoleNames()[0] != 'admin' && Auth::user()->getRoleNames()[0] === 'employer') {
            ...
            if ($type === 'project') {
                $invoices = DB::table('invoices')
                    ...
                    ->get();
                return view('job.back-end.employer.invoices.project', compact('invoices', 'type', 'expiry_date', 'symbol'));
            } elseif ($type === 'package') {
                $invoices = DB::table('invoices')
                    ->join('items', 'items.invoice_id', '=', 'invoices.id')
                    ...
                    ->where('invoices.type', $type)
                    ->get();
                return view('job.back-end.employer.invoices.package', compact('invoices', 'type', 'expiry_date', 'symbol'));
            }
        }
    }
    ...
    public function getFreelancerInvoices($type = '')
    {
        if (Auth::user()->getRoleNames()[0] != 'admin' && Auth::user()->getRoleNames()[0] === 'freelancer') {
            ...
            $symbol = !empty($currency) && !empty($currency[0]['currency']) ? JobHelper::currencyList($currency[0]['currency']) : array();
            if ($type === 'project') {
                return view('job.back-end.freelancer.invoices.project', compact('invoices', 'type', 'expiry_date', 'symbol'));
            } elseif ($type === 'package') {
                return view('job.back-end.freelancer.invoices.package', compact('invoices', 'type', 'expiry_date', 'symbol'));
            }
        } else {
            abort(404);
        }
    ...
    public function showInvoice($id)
    {
        if (!empty($id)) {
            ...
            $code = JobHelper::currencyList($currency_code);
            $symbol = !empty($code) ? $code['symbol'] : '$';
            if (Auth::user()->getRoleNames()->first() === 'freelancer') {
                return view::make('job.back-end.freelancer.invoices.show', compact('invoice_info', 'symbol', 'currency_code'));
            } elseif (Auth::user()->getRoleNames()->first() === 'employer') {
                return view::make('job.back-end.employer.invoices.show', compact('invoice_info', 'symbol', 'currency_code'));
            }
        }
    }
    ...
    {
        $profile = JobProfile::where('user_id', Auth::user()->id)
            ->get()->first();
        ...
        return view(
            'job.back-end.admin.profile-settings.personal-detail.index',
            compact(
                'banner',
                'avater',
                'tagline',
                ...
                if ($storeProjectOffers == "success") {
                    ...
                    if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                        ...
                        if (!empty($send_freelancer_offer->id)) {
                            $job = JobJob::where('id', $request['offer']['project'])->first();
    ...
    {
        $job = JobJob::where('slug', $slug)->first();
        $reasons = JobHelper::getReportReasons();
        return View(
            'job.back-end.freelancer.jobs.dispute',
            compact(
                'job',
                'reasons'
            )
        ...
        if ($storeDispute == "success") {
            ...
            if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                ...
                if (!empty($dispute_raised_template->id)) {
                    ...
                    $email_params['project_title'] = $job->title;
                    $email_params['project_link'] = url('Job/job/' . $job->slug);
                    $email_params['sender_link'] = url('Job/profile/' . $user->slug);
                    $email_params['name'] = JobHelper::getUserName(Auth::user()->id);
                    $email_params['msg'] = $request['description'];
    ...
    public function userListing()
    {
        if (Auth::user() && Auth::user()->getRoleNames()->first() === 'admin') {
            ...
            return view('job.back-end.admin.users.index', compact('users'));
        } else {
            abort(404);
        }
    }
    ...
    {
        if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
            $random_number = JobHelper::generateRandomCode(4);
            $verification_code = strtoupper($random_number);
            $user = User::find(Auth::user()->id);
            $user->verification_code = $verification_code;
            $user->save();
            $register_form = JobSiteManagement::getMetaValue('reg_form_settings');
            ...
            return View('job.back-end.email-verify.index', compact('reg_three_title', 'reg_three_subtitle', 'register_image'));
        } else {
    ...
    public function verifyUserEmailCode(Request $request)
    {
        ...
        if (!empty($request['code'])) {
            if ($request['code'] === $user->verification_code) {
                ...
                $user->save();
                return Redirect::to('Job/'.$role.'/dashboard');
            } else {
                Session::flash('error', trans('lang.ph_email_warning'));
                return Redirect::back();
            }
    ...
    public function activate_pro($id){
        $id=base64_decode($id);
        $user=User::find($id);
        $role_id=Role::where('name','candidate')->select('id')->first();
