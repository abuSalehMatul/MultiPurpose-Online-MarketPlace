 ...
 *
 */
class JobSiteManagementController extends Controller
{
     ...
     * @return void
     */
    public function __construct(JobSiteManagement $settings)
    {
    ...
    public function settings()
    {
        ...
        $social_list = JobHelper::getSocialData();
        $social_unserialize_array = JobSiteManagement::getMetaValue('socials');
        $unserialize_menu_array = JobSiteManagement::getMetaValue('search_menu');
        $menu_title = DB::table('job_site_managements')->select('meta_value')->where('meta_key', 'menu_title')->get()->first();
        $currency = array_pluck(JobHelper::currencyList(), 'code', 'code');
        $payment_methods = JobHelper::getPaymentMethodList();
        ...
        $languages = JobHelper::getTranslatedLang();
        ...
        $icons = JobHelper::getIconList();
        $dash_icons  = JobSiteManagement::getMetaValue('icons');
        return view(
            'job.back-end.admin.settings.index',
            compact(
    ...
    public function homePageSettings()
    {
        ...
        return view(
            'job.back-end.admin.home-page-settings.index',
            compact(
    ...
    public function storeHomeSettings(Request $request)
    {
        ...
        $json = array();
        if (!empty($request)) {
            $store_home_settings = JobSiteManagement::saveHomeSettings($request['home'], $request);
    ...
    public function storeSectionSettings(Request $request)
    {
        ...
        $json = array();
        if (!empty($request)) {
            $store_home_settings = JobSiteManagement::saveSectionSettings($request['section'], $request);
            if ($store_home_settings == "success") {
                $json['type'] = 'success';
                $json['progressing'] = trans('lang.saving');
                $json['message'] = trans('lang.settings_saved');
    ...
    {
        $path = JobHelper::PublicPath() . '/uploads/settings/temp/';
        if (!empty($request[$file_name])) {
            JobHelper::uploadSingleTempImage($path, $request[$file_name]);
        }
    }
    ...
    public function importDemo()
    {
        ...
        if (!empty($server_verification)) {
            ...
            return redirect()->to('Job/admin/settings');
        }
    ...
    {
        $json = array();
        $settings = DB::table('job_site_managements')->select('meta_value')->where('meta_key', 'show-page-'.$request['page_id'])->get()->first();
    ...
    {
        $json = array();
        $section_settings = !empty(JobSiteManagement::getMetaValue('section_settings')) ? JobSiteManagement::getMetaValue('section_settings') : array();
