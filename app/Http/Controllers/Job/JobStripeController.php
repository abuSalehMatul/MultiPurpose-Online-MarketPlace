 ...
 *
 */
class JobStripeController extends Controller
{
    /**
    ...
    {
        return view('job.back-end.paymentstripe');
    }
    /**
    ...
    public function postPaymentWithStripe(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            try {
                ...
                if ($payment_detail['status'] == 'succeeded') {
                    ...
                    if ($type == 'package') {
                        $item = DB::table('job_items')->select('id')->where('subscriber', $user_id)->first();
                        if (!empty($item)) {
                    ...
                    if (Auth::user()) {
                        if ($type == 'package') {
                            if (session()->has('product_id')) {
                                $package_item = \App\JobModel\JobItem::where('subscriber', $user_id)->first();
                            ...
                            $proposal = JobProposal::find($product_id);
                            $proposal->hired = 1;
        ...
        if (Auth::user()->getRoleNames()[0] == "employer") {
            if ($type == 'project') {
                ...
                $json['url'] = url('Job/employer/jobs/hired');
                return $json;
            } else {
                $json['type'] = 'success';
                $json['message'] = trans('lang.thanks_subscription');
                $json['url'] = url('Job/dashboard/packages/employer');
                return $json;
            }
        } else if (Auth::user()->getRoleNames()[0] == "freelancer") {
            $json['type'] = 'success';
            $json['message'] = trans('lang.thanks_subscription');
            $json['url'] = url('Job/dashboard/packages/freelancer');
            return $json;
        }
    }
}
