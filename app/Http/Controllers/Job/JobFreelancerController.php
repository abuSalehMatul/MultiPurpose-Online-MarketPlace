 ...
 *
 */
class JobFreelancerController extends Controller
{
    /**
     ...
     * @return void
     */
    public function __construct(JobProfile $freelancer, Payout $payout)
    {
    ...
    public function index()
    {
        ...
        $skills = JobSkill::pluck('title', 'id');
        ...
        $role_id =  JobHelper::getRoleByUserID(Auth::user()->id);
        ...
        return view(
            'job.back-end.freelancer.profile-settings.personal-detail.index',
            compact(
                'locations',
                'skills',
                'profile',
    ...
    {
        return view('job.back-end.freelancer.profile-settings.experience-education.index');
    }
    ...
    public function projectAwardsSettings()
    {
        ...
        return view('job.back-end.freelancer.profile-settings.projects-awards.index');
    }
    ...
    public function paymentSettings()
    {
        ...
        return view('job.back-end.freelancer.profile-settings.payment-settings.index', compact('payout_id'));
    }
    ...
    public function showFreelancerJobs($status)
    {
        ...
        if (Auth::user()) {
            $ongoing_jobs = JobProposal::select('job_id')->latest()->where('freelancer_id', $freelancer_id)->where('status', 'hired')->paginate(7);
            $completed_jobs = JobProposal::select('job_id')->latest()->where('freelancer_id', $freelancer_id)->where('status', 'completed')->paginate(7);
            $cancelled_jobs = JobProposal::select('job_id')->latest()->where('freelancer_id', $freelancer_id)->where('status', 'cancelled')->paginate(7);
            if (!empty($status) && $status === 'hired') {
                return view(
                    'job.back-end.freelancer.jobs.ongoing',
                    compact(
                        'ongoing_jobs', 'symbol'
                    )
                );
                ...
                return view(
                    'job.back-end.freelancer.jobs.completed',
                    compact(
                ...
                return view(
                    'job.back-end.freelancer.jobs.cancelled',
                    compact(
    ...
    public function showOnGoingJobDetail($slug)
    {
        ...
        if (Auth::user()) {
            ...
            $proposal = JobJob::find($job->id)->proposals()->select('id', 'status')->where('status', '!=', 'pending')
                ->first();
            $employer_name = JobHelper::getUserName($job->user_id);
            $duration = !empty($job->duration) ? JobHelper::getJobDurationList($job->duration) : '';
            ...
            $symbol = !empty($currency) && !empty($currency[0]['currency']) ? JobHelper::currencyList($currency[0]['currency']) : array();
            return view(
                'job.back-end.freelancer.jobs.show',
                compact(
                    'job',
                    'employer_name',
                    'duration',
    ...
    {
        $proposals = JobProposal::select('job_id', 'status', 'id')->where('freelancer_id', Auth::user()->id)->latest()->paginate(7);
        $currency  = JobSiteManagement::getMetaValue('commision');
        $symbol    = !empty($currency) && !empty($currency[0]['currency']) ? JobHelper::currencyList($currency[0]['currency']) : array();
        return view(
            'job.back-end.freelancer.proposals.index',
            compact(
                'proposals', 'symbol'
            )
        );
    ...
    public function freelancerDashboard()
    {
        $ongoing_jobs = array();
        $freelancer_id = Auth::user()->id;
        $ongoing_projects = JobProposal::getProposalsByStatus($freelancer_id, 'hired', 3);
        $cancelled_projects = JobProposal::getProposalsByStatus($freelancer_id, 'cancelled');
        $package_item = JobItem::where('subscriber', $freelancer_id)->first();
        ...
        $completed_projects = JobProposal::getProposalsByStatus($freelancer_id, 'completed');
        $currency   = JobSiteManagement::getMetaValue('commision');
        $symbol     = !empty($currency) && !empty($currency[0]['currency']) ? JobHelper::currencyList($currency[0]['currency']) : array();
        ...
        if (Auth::user()) {
            return view(
                'job.back-end.freelancer.dashboard',
                compact(
                    'ongoing_projects',
                    'cancelled_projects',
                    'expiry_date',
    ...
    public function getPayouts()
    {
        ...
        return view(
            'job.back-end.freelancer.payouts',
            compact('payouts')
        );
    }
