...
class JobPublicController extends Controller
{
                ...
                if ($request['code'] === $user->verification_code) {
                    ...
                    if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                        ...
                        if (!empty($template->id)) {
                            ...
                            $email_params['name'] = JobHelper::getUserName($id);
    ...
    public function showUserProfile($slug)
    {
        ...
        if (!empty($user)) {
            ...
            $job = JobJob::where('user_id', $user->id)->get();
            $profile = JobProfile::all()->where('user_id', $user->id)->first();
            ...
            $banner = !empty($profile->banner) ? '/uploads/users/' . $profile->user_id . '/' . $profile->banner : JobHelper::getUserProfileBanner($user->id);
            $auth_user = Auth::user() ? true : false;
            $user_name = JobHelper::getUserName($profile->user_id);
            ...
            if ($user->getRoleNames()->first() === 'freelancer') {
                ...
                $jobs =JobJob::select('title', 'id')->get()->pluck('title', 'id');
                ...
                $employer_projects = Auth::user() ? JobHelper::getEmployerJobs(Auth::user()->id) : array();
                $payment_settings = JobSiteManagement::getMetaValue('commision');
                $currency_symbol  = !empty($payment_settings) && !empty($payment_settings[0]['currency']) ? JobHelper::currencyList($payment_settings[0]['currency']) : array();
                $symbol = !empty($currency_symbol['symbol']) ? $currency_symbol['symbol'] : '$' ;
                $settings = !empty(JobSiteManagement::getMetaValue('settings')) ? JobSiteManagement::getMetaValue('settings') : array();
                $display_chat = !empty($settings[0]['chat_display']) ? $settings[0]['chat_display'] : false;
                return View(
                    'job.front-end.users.freelancer-show',
                    compact(
                        'profile',
                        'amount',
                        'skills',
                ...
                $jobs = JobJob::where('user_id', $profile->user_id)->latest()->paginate(7);
                ...
                return View(
                    'job.front-end.users.employer-show',
                    compact(
                        'profile',
                        'skills',
                        'user',
    ...
    {
        $json = array();
        $filters = JobHelper::getSearchFilterList();
        if (!empty($filters)) {
            $json['type'] = 'success';
            $json['result'] = $filters;
            return $json;
    ...
    public function getSearchableData(Request $request)
    {
        ...
        if (!empty($request['type'])) {
            $searchables = JobHelper::getSearchableList($request['type']);
    ...
    public function getSearchResult($search_type = "")
    {
        ...
        $currency   = JobSiteManagement::getMetaValue('commision');
        $symbol     = !empty($currency) && !empty($currency[0]['currency']) ? JobHelper::currencyList($currency[0]['currency']) : array();
        $freelancer_skills = JobHelper::getFreelancerLevelList();
        $project_length = JobHelper::getJobDurationList();
        $Jobs_total_records = JobJob::count();
               ...
               // return $type;
                if ($type == 'job_employer' || $type== 'candidate' ) {
                   // return $type;
                    ...
                    $save_freelancer = !empty(auth()->user()->job_profile->saved_freelancer) ?
                        unserialize(auth()->user()->job_profile->saved_freelancer) : array();
                    $save_employer = !empty(auth()->user()->job_profile->saved_employers) ?
                        unserialize(auth()->user()->job_profile->saved_employers) : array();
                } else {
                    abort(404);
                }
                ...
                elseif ($type === 'freelancer') {
                   // return $project_length;
                    return view(
                        'job.front-end.freelancers.index',
                        compact(
                            'type',
                            'users',
                            'categories',
                ...
                $results = JobJob::getSearchResult(
                    $keyword,
                    $search_categories,
                    $search_locations,
                    $search_skills,
    ...
    public function resetPasswordView($verification_code)
    {
        if (!empty($verification_code)) {
            ...
            return View('job.front-end.reset-password');
        } else {
            abort(404);
        }
    }
