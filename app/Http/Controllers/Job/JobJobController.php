 ...
 *
 */
class JobJobController extends Controller
{
    /**
     ...
     * @return void
     */
    public function __construct(JobJob $job)
    {
        $this->job = $job;
    }
    ...
    public function postJob()
    {
        ...
        $project_levels = JobHelper::getProjectLevel();
        $job_duration = JobHelper::getJobDurationList();
        ...
        return view(
            'job.back-end.employer.jobs.create',
            compact(
                'english_levels',
                'languages',
                'project_levels',
    ...
    public function index()
    {
        ...
        return view('job.back-end.employer.jobs.index', compact('job_details', 'symbol'));
    }
    ...
    {
        if (!empty($job_slug)) {
            $job = JobJob::where('slug', $job_slug)->first();
            $json = array();
            $languages = Language::pluck('title', 'id');
            $locations = JobLocation::pluck('title', 'id');
            $skills = JobSkill::pluck('title', 'id');
            ...
            $english_levels = JobHelper::getEnglishLevelList();
            $job_duration = JobHelper::getJobDurationList();
            $freelancer_level_list = JobHelper::getFreelancerLevelList();
            $attachments = !empty($job->attachments) ? unserialize($job->attachments) : '';
            if (!empty($job)) {
                return View(
                    'job.back-end.employer.jobs.edit',
                    compact(
                        'job',
                        'project_levels',
                        'english_levels',
    ...
    public function getAttachmentSettings(Request $request)
    {
        $json = array();
        if ($request['slug']) {
            $settings = JobJob::where('slug', $request['slug'])
    ...
    public function store(Request $request)
    {
      ...
      //  return $json;
        ...
        $package_item = JobItem::where('subscriber', Auth::user()->id)->first();
        $package = !empty($package_item) ? JobPackage::find($package_item->product_id) : '';
        ...
        $posted_featured_jobs = JobJob::where('user_id', Auth::user()->id)->where('is_featured', 'true')->count();
        if ($package != '' && $current_date <= $expiry_date) {
            ...
            if ($posted_jobs >= intval($option['jobs'])) {
                ...
                if ($job_post = 'success') {
                    ...
                    return redirect( 'Job/employer/dashboard/manage-jobs');
                }
            }
        } else {
            $json['type'] = 'error';
