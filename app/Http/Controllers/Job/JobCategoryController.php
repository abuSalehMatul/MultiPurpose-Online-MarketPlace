...
class JobCategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return View::make(
            'job.back-end.admin.categories.index', compact('cats')
        );
    }
    ...
    public function edit($id)
    {
        if (!empty($id)) {
            ...
            if (!empty($cats)) {
                return View::make(
                    'job.back-end.admin.categories.edit', compact('id', 'cats')
                );
                return Redirect::to('Job/admin/categories');
            }
        }
    }
    ...
    {
        $path = JobHelper::PublicPath() . '/uploads/categories/temp/';
        if (!empty($request['uploaded_image'])) {
            return JobHelper::uploadTempImage($path, $request['uploaded_image']);
        }
    }
    ...
    public function categoriesList(Request $request)
    {
        ...
        if (!empty($categories)) {
            return View::make(
                'job.front-end.categories.index',
                compact(
                    'categories'
                )
            );
