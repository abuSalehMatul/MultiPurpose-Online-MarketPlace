 ...
 *
 */
class ProFreelancerController extends Controller
{
    /**
     ...
     * @return void
     */
    public function __construct(ProProfile $freelancer, Payout $payout)
    {
    ...
    {
        $locations = ProLocation::pluck('title', 'id');
        $skills = ProSkill::pluck('title', 'id');
        ...
        $role_id =  ProHelper::getRoleByUserID(Auth::user()->id);
        $packages = DB::table('pro_items')->where('subscriber', Auth::user()->id)->count();
        ...
        return view(
            'pro.back-end.freelancer.profile-settings.personal-detail.index',
            compact(
                'locations',
                'skills',
                'profile',
    ...
    {
        return view('pro.back-end.freelancer.profile-settings.experience-education.index');
    }
    ...
    public function projectAwardsSettings()
    {
        ...
        return view('pro.back-end.freelancer.profile-settings.projects-awards.index');
    }
    ...
    public function paymentSettings()
    {
        ...
        return view('pro.back-end.freelancer.profile-settings.payment-settings.index', compact('payout_id'));
    }
    ...
    public function showFreelancerJobs($status)
    {
        ...
        if (Auth::user()) {
            $ongoing_jobs = ProProposal::select('job_id')->latest()->where('freelancer_id', $freelancer_id)->where('status', 'hired')->paginate(7);
            $completed_jobs = ProProposal::select('job_id')->latest()->where('freelancer_id', $freelancer_id)->where('status', 'completed')->paginate(7);
            $cancelled_jobs = ProProposal::select('job_id')->latest()->where('freelancer_id', $freelancer_id)->where('status', 'cancelled')->paginate(7);
            if (!empty($status) && $status === 'hired') {
                return view(
                    'pro.back-end.freelancer.jobs.ongoing',
                    compact(
                        'ongoing_jobs', 'symbol'
                    )
                );
                ...
                return view(
                    'pro.back-end.freelancer.jobs.completed',
                    compact(
                ...
                return view(
                    'pro.back-end.freelancer.jobs.cancelled',
                    compact(
    ...
    public function showOnGoingJobDetail($slug)
    {
        ...
        if (Auth::user()) {
            ...
            $proposal = ProJob::find($job->id)->proposals()->select('id', 'status')->where('status', '!=', 'pending')
                ->first();
            $employer_name = ProHelper::getUserName($job->user_id);
            $duration = !empty($job->duration) ? ProHelper::getJobDurationList($job->duration) : '';
            ...
            $symbol = !empty($currency) && !empty($currency[0]['currency']) ? ProHelper::currencyList($currency[0]['currency']) : array();
            return view(
                'pro.back-end.freelancer.jobs.show',
                compact(
                    'job',
                    'employer_name',
                    'duration',
    ...
    {
        $proposals = ProProposal::select('job_id', 'status', 'id')->where('freelancer_id', Auth::user()->id)->latest()->paginate(7);
        $currency  = ProSiteManagement::getMetaValue('commision');
        $symbol    = !empty($currency) && !empty($currency[0]['currency']) ? ProHelper::currencyList($currency[0]['currency']) : array();
        return view(
            'pro.back-end.freelancer.proposals.index',
            compact(
                'proposals', 'symbol'
            )
        );
    ...
    public function freelancerDashboard()
    {
        $ongoing_jobs = array();
        $freelancer_id = Auth::user()->id;
        $ongoing_projects = ProProposal::getProposalsByStatus($freelancer_id, 'hired', 3);
        $cancelled_projects = ProProposal::getProposalsByStatus($freelancer_id, 'cancelled');
        $package_item = ProItem::where('subscriber', $freelancer_id)->first();
        $package = !empty($package_item) ? ProPackage::find($package_item->product_id) : array();
        ...
        $completed_projects = ProProposal::getProposalsByStatus($freelancer_id, 'completed');
        $currency   = ProSiteManagement::getMetaValue('commision');
        $symbol     = !empty($currency) && !empty($currency[0]['currency']) ? ProHelper::currencyList($currency[0]['currency']) : array();
        $trail      = !empty($package) && $package['trial'] == 1 ? 'true' : 'false';
        $icons      = ProSiteManagement::getMetaValue('icons');
        ...
        if (Auth::user()) {
            return view(
                'pro.back-end.freelancer.dashboard',
                compact(
                    'ongoing_projects',
                    'cancelled_projects',
                    'expiry_date',
    ...
    public function getPayouts()
    {
        ...
        return view(
            'pro.back-end.freelancer.payouts',
            compact('payouts')
        );
    }
