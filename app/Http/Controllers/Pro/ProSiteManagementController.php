 ...
 *
 */
class ProSiteManagementController extends Controller
{
     ...
     * @return void
     */
    public function __construct(ProSiteManagement $settings)
    {
    ...
    public function settings()
    {
        ...
        $unserialize_menu_array = ProSiteManagement::getMetaValue('search_menu');
        $menu_title = DB::table('pro_site_managements')->select('meta_value')->where('meta_key', 'menu_title')->get()->first();
        $currency = array_pluck(ProHelper::currencyList(), 'code', 'code');
        $payment_methods = ProHelper::getPaymentMethodList();
        ...
        $languages = ProHelper::getTranslatedLang();
        ...
        $icons = ProHelper::getIconList();
        $dash_icons  = ProSiteManagement::getMetaValue('icons');
        return view(
            'pro.back-end.admin.settings.index',
            compact(
    ...
    public function homePageSettings()
    {
        ...
        return view(
            'pro.back-end.admin.home-page-settings.index',
            compact(
    ...
    public function storeHomeSettings(Request $request)
    {
        ...
        $json = array();
        if (!empty($request)) {
            $store_home_settings = ProSiteManagement::saveHomeSettings($request['home'], $request);
    ...
    public function storeSectionSettings(Request $request)
    {
        ...
        $json = array();
        if (!empty($request)) {
            $store_home_settings = ProSiteManagement::saveSectionSettings($request['section'], $request);
            if ($store_home_settings == "success") {
                $json['type'] = 'success';
                $json['progressing'] = trans('lang.saving');
                $json['message'] = trans('lang.settings_saved');
    ...
    {
        $path = ProHelper::PublicPath() . '/uploads/settings/temp/';
        if (!empty($request[$file_name])) {
            ProHelper::uploadSingleTempImage($path, $request[$file_name]);
        }
    }
    ...
    public function importDemo()
    {
        ...
        if (!empty($server_verification)) {
            ...
            return redirect()->to('Pro/admin/settings');
        }
    ...
    {
        $json = array();
        $settings = DB::table('pro_site_managements')->select('meta_value')->where('meta_key', 'show-page-'.$request['page_id'])->get()->first();
