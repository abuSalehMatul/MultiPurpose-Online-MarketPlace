 ...
 * Class EmployerController
 */
class ProEmployerController extends Controller
{
     ...
     * @return void
     */
    public function __construct(ProProfile $employer)
    {
    ...
    public function index()
    {
        $profile = $this->employer::where('user_id', Auth::user()->id)
            ->get()->first();
        $employees = ProHelper::getEmployeesList();
        $departments = ProDepartment::all();
        ...
        $package_options = ProPackage::select('options')->where('role_id', Auth::user()->id)->first();
        $options = !empty($package_options) ? unserialize($package_options['options']) : array();
        return view(
            'pro.back-end.employer.profile-settings.personal-detail.index',
            compact(
                'employees',
                'departments',
                'locations',
    ...
    public function uploadTempImage(Request $request)
    {
        ...
        if (!empty($request['hidden_avater_image'])) {
            ...
            return ProHelper::uploadTempImageWithSize($path, $profile_image, '', $image_size);
        }
    }
    ...
    public function employerDashboard()
    {
        ...
        $package_item = ProItem::where('subscriber', $employer_id)->first();
        ...
        if (Auth::user()) {
            ...
            return view(
                'pro.back-end.employer.dashboard',
                compact(
                    'ongoing_jobs',
                    'expiry_date',
                    'notify_class',
    ...
    public function showEmployerJobs($status)
    {
        ...
        $employer_id = Auth::user()->id;
        if (Auth::user()) {
            $ongoing_jobs = ProJob::where('user_id', $employer_id)->latest()->where('status', 'hired')->paginate(7);
            $completed_jobs = ProJob::where('user_id', $employer_id)->latest()->where('status', 'completed')->paginate(7);
            $cancelled_jobs = ProJob::where('user_id', $employer_id)->latest()->where('status', 'cancelled')->paginate(7);
            $currency   = ProSiteManagement::getMetaValue('commision');
            $symbol = !empty($currency) && !empty($currency[0]['currency']) ? ProHelper::currencyList($currency[0]['currency']) : array();
            if (!empty($status) && $status === 'hired') {
                return view(
                    'pro.back-end.employer.jobs.ongoing',
                    compact(
                        'ongoing_jobs', 'symbol'
                    )
                );
                ...
                return view(
                    'pro.back-end.employer.jobs.completed',
                    compact(
    ...
    public function employerPaymentProcess($id)
    {
        if (Auth::user() && !empty($id)) {
            if (Auth::user()) {
                ...
                $proposal = ProProposal::where('id', $id)->get()->first();
                $job = $proposal->job;
                $freelancer = User::find($proposal->freelancer_id);
                $freelancer_name = ProHelper::getUserName($proposal->freelancer_id);
                ...
                $symbol = !empty($currency) && !empty($currency[0]['currency']) ? ProHelper::currencyList($currency[0]['currency']) : array();
                return view(
                    'pro.back-end.employer.jobs.checkout',
                    compact(
                        'job',
                        'freelancer_name',
                        'profile_image',
