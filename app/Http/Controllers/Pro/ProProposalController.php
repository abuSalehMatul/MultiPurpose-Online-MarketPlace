 ...
 *
 */
class ProProposalController extends Controller
{
    /**
     ...
     * @return void
     */
    public function __construct(ProProposal $proposal)
    {
    ...
    {
        if (!empty($job_slug)) {
            $job = ProJob::all()->where('slug', $job_slug)->first();
            if (!empty($job)) {
                $job_skills = $job->skills->pluck('id')->toArray();
                $check_skill_req = $this->proposal->getJobSkillRequirement($job_skills);
                $proposal_status = ProJob::find($job->id)->proposals()->where('status', 'hired')->first();
                ...
                $duration =  ProHelper::getJobDurationList($job->duration);
                $job_completion_time = ProHelper::getJobDurationList();
                $commision_amount = ProSiteManagement::getMetaValue('commision');
    ...
    public function uploadTempImage(Request $request)
    {
        if (!empty($request['file'])) {
            ...
            return ProHelper::uploadTempMultipleAttachments($attachments, $path);
        }
    }
    ...
    public function store(Request $request)
    {
        if (Auth::user()) {
            ...
            if (!empty($request)) {
                ...
                $package = DB::table('pro_items')->where('subscriber', Auth::user()->id)->select('product_id')->first();
                            ...
                            if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                                if (!empty($job->employer->email)) {
                                    ...
                                    if (
                                        ...
                                        $email_params['freelancer'] = ProHelper::getUserName(Auth::user()->id);
                                        ...
                                        $email_params['duration'] = ProHelper::getJobDurationList($request['completion_time']);
    ...
    public function getJobProposals($slug)
    {
        if (!empty($slug)) {
            ...
            $proposals = ProJob::latest()->find($job->id)->proposals;
            $accepted_proposal = ProJob::find($job->id)->proposals()->where('hired', 1)->first();
            $duration = !empty($job->duration) ? ProHelper::getJobDurationList($job->duration) : '';
            $currency   = ProSiteManagement::getMetaValue('commision');
            $symbol = !empty($currency) && !empty($currency[0]['currency']) ? ProHelper::currencyList($currency[0]['currency']) : array();
            return View(
                'pro.back-end.employer.proposals.index',
                compact(
                    'proposals',
                    'job',
                    'duration',
    ...
    public function show($slug)
    {
        ...
        $attachments = array();
        $job = ProJob::where('slug', $slug)->first();
        $accepted_proposal = ProJob::find($job->id)->proposals()->where('hired', 1)
            ->first();
        $freelancer_name = ProHelper::getUserName($accepted_proposal->freelancer_id);
        $completion_time = !empty($accepted_proposal->completion_time) ?
            ProHelper::getJobDurationList($accepted_proposal->completion_time) : '';
        ...
        $project_status = ProHelper::getProjectStatus();
        $duration = !empty($job->duration) ? ProHelper::getJobDurationList($job->duration) : '';
        ...
        $symbol = !empty($currency) && !empty($currency[0]['currency']) ? ProHelper::currencyList($currency[0]['currency']) : array();
        return view(
            'pro.back-end.employer.proposals.show',
            compact(
                'job',
                'duration',
                'accepted_proposal',
