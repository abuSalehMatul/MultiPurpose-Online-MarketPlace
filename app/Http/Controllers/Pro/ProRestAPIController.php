...
use DB;
use App\Badge;
use App\ProModel\ProSkill;
use App\Language;
use Auth;
use File;
use Storage;
use App\ProModel\ProProposal;
use App\ProModel\ProSiteManagement;
use App\ProModel\ProLocation;
use App\EmailTemplate;
use App\ProModel\ProCategory;
use App\ProModel\ProDepartment;
 ...
 *
 */
class ProRestAPIController extends Controller
{
    ...
    {
        $json = array();
        $currency   = ProSiteManagement::getMetaValue('commision');
        $curr_symbol = !empty($currency) && !empty($currency[0]['currency']) ? ProHelper::currencyList($currency[0]['currency']) : array();
        ...
        if (!empty($login_user) && !empty($type)) {
            ...
            if ($type === 'featured') {
                $subscribers =  DB::table('pro_packages')
                    ->join('pro_items', 'pro_items.product_id', '=', 'packages.id')
                    ->join('invoices', 'invoices.id', '=', 'pro_items.invoice_id')
                    ->select('pro_items.subscriber')
                    ->where('invoices.type', '!=', 'project')
                    ->where('packages.badge_id', '!=', 0)
                    ->pluck('pro_items.subscriber')->toArray();
            ...
            if (!empty($users) && $users->count() > 0) {
                foreach ($users as $key => $user) {
                    ...
                    $json[$key]['name'] = ProHelper::getUserName($user['id']);
                    $json[$key]['freelancer_link'] = url('Pro/profile/' . $user_object->slug);
                    ...
                    $json[$key]['profile_img'] = url(ProHelper::getProfileImage($user['id']));
                    $json[$key]['banner_img'] = url(ProHelper::getProfileBanner($user['id']));
                    $badge = ProHelper::getUserBadge($user['id']);
                    ...
                    $json[$key]['completed_jobs'] = ProHelper::getProposals($user['id'], 'completed')->count();
                    $json[$key]['ongoning_jobs'] = ProHelper::getProposals($user['id'], 'hired')->count();
                    $json[$key]['cancelled_jobs'] = ProHelper::getProposals($user['id'], 'cancelled')->count();
                    $reviews = Review::where('receiver_id', $user['id'])->get();
                    $freelancer_reviewes  = array();
                    if (!empty($reviews) && $reviews->count() > 0) {
                        foreach ($reviews as $reviews_key => $review) {
                            $job = ProJob::where('id', $review->job_id)->first();
                            ...
                            $freelancer_reviewes[$reviews_key]['employer_image'] = !empty(ProHelper::getProfileImage($review->user_id)) ? asset(ProHelper::getProfileImage($review->user_id)) : '';
                            $freelancer_reviewes[$reviews_key]['_is_verified'] = $verified_user == 1 ? 'yes' : 'no';
                            $freelancer_reviewes[$reviews_key]['employer_name'] = ProHelper::getUserName($review->user_id);
                            $freelancer_reviewes[$reviews_key]['level_title'] = !empty($job->project_level) ?  ProHelper::getProjectLevel($job->project_level) : '';
    ...
    public function userLogin(Request $request)
    {
        ...
        if (auth()->attempt($credentials)) {
            ...
            $json['profile']['pmeta']['profile_img'] = url(ProHelper::getProfileImage(auth()->user()->id));
            $json['profile']['pmeta']['banner_img'] = url(ProHelper::getProfileBanner(auth()->user()->id));
            ...
            $json['profile']['pmeta']['full_name'] = ProHelper::getUserName(auth()->user()->id);
    ...
    public function uploadMedia(Request $request)
    {
        ...
        $path = ProHelper::PublicPath() . '/uploads/users/' . $user_id;
        $file_name = time() . '-' . $image->getClientOriginalName();
        if (!empty($user_id) && !empty($image)) {
            $upload_image = ProHelper::uploadTempImage($path, $image, $file_name);
            if ($upload_image['type'] == 'success') {
                $user_profile = ProProfile::select('id')->where('user_id', $user_id)
                    ->get()->first();
                if (!empty($user_profile->id)) {
                    $profile = ProProfile::find($user_profile->id);
                    if ($request->file('profile_image')) {
    ...
    public function submitProposal(Request $request)
    {
        ...
        if (!empty($request)) {
            ...
            if (!empty($request['user_id']) && !empty($request['project_id']) && !empty($request['proposed_amount']) && !empty($request['proposed_time']) && !empty($request['proposed_content'])) {
                $job = ProJob::find(intval($project_id));
                if (intval($amount) > $job->price) {
                    $json['type'] = 'error';
                    $json['message'] = 'Proposal amount must not exceed the job amount';
                    return Response::json($json, 203);
                }
                $package = DB::table('pro_items')->where('subscriber', $user_id)->select('product_id')->first();
                $proposal = new ProProposal();
                $proposals = $proposal::where('freelancer_id', $user_id)->count();
                $settings = ProSiteManagement::getMetaValue('settings');
                ...
                if (!empty($package->product_id)) {
                    $package_options = ProPackage::select('options')->where('id', $package->product_id)->get()->first();
                    ...
                    if ($proposals <= $allowed_proposals) {
                        ...
                        // Send Email
                        $email_settings = ProSiteManagement::getMetaValue('settings');
                        ...
                        if (!empty($job->employer->email)) {
                            ...
                            if (
                                ...
                                $email_params['employer'] = ProHelper::getUserName($job->employer->id);
                                $email_params['employer_profile'] = url('Pro/profile/' . $job->employer->slug);
                                $email_params['freelancer'] = ProHelper::getUserName($user_id);
                                $email_params['freelancer_profile'] = url('Pro/profile/' . $user->slug);
                                $email_params['title'] = $job->title;
                                $email_params['link'] = url('Pro/job/' . $job->slug);
                                $email_params['amount'] = $amount;
                                $email_params['duration'] = ProHelper::getJobDurationList($duration);
    ...
    public function getEmployer()
    {
        ...
        if ($type === 'latest') {
            ...
            if (!empty($search_project_types)) {
                ...
                $projects = ProJob::where('project_type', $search_project_types)->get();
                if ($projects->count() > 0) {
        ...
        if (!empty($users) && $users->count() > 0) {
            foreach ($users as $key => $user) {
                ...
                $user_object = User::find($user['id']);
                $json[$key]['company_link'] = !empty($user_object) ? url('Pro/profile/' . $user_object->slug) : '';
                ...
                $json[$key]['profile_img'] = !empty($user_object->profile->avater) ? url(ProHelper::getProfileImage($user_object->id)) : '';
                ...
                $json[$key]['banner_img'] = !empty($user_object->profile->banner) ? url(ProHelper::getProfileBanner($user_object->id)) : '';
                $json[$key]['link'] = !empty($user_object->id) ? url('Pro/profile/' . $user['slug'] . '') : '';
                $json[$key]['_longitude'] = !empty($user_object->profile->longitude) ? $user_object->profile->longitude : '';
                $json[$key]['_latitude'] = !empty($user_object->profile->latitude) ? $user_object->profile->latitude : '';
                $json[$key]['_address'] = !empty($user_object->profile->address) ? $user_object->profile->address : '';
                $json[$key]['_tag_line'] = !empty($user_object->profile->tagline) ? $user_object->profile->tagline : '';
    ...
    public function getJobs(Request $request)
    {
        ...
        if ($type === 'latest') {
            ...
            if (!empty($search_categories)) {
                foreach ($search_categories as $key => $search_category) {
                    $categor_obj = ProCategory::where('slug', $search_category)->first();
                    $category = ProCategory::find($categor_obj->id);
            ...
            if (!empty($search_category)) {
                $category_obj = ProCategory::where('slug', $search_category)->first();
                $category = ProCategory::find($category_obj->id);
                if (!empty($category->jobs)) {
                    $category_jobs = $category->jobs->pluck('id')->toArray();
                    foreach ($category_jobs as $id) {
                        $job_id[] = $id;
            ...
            if (!empty($search_locations)) {
                $filters['locations'] = $search_locations;
                $locations = ProLocation::select('id')->whereIn('slug', $search_locations)->get()->pluck('id')->toArray();
                $jobs->whereIn('location_id', $locations);
            }
            if (!empty($search_skills)) {
                $filters['skills'] = $search_skills;
                foreach ($search_skills as $key => $search_skill) {
                    $skill_obj = ProSkill::where('slug', $search_skill)->first();
                    $skill = ProSkill::find($skill_obj->id);
                    if (!empty($skill->jobs)) {
        ...
        if (!empty($user) && $user->count() > 0) {
            foreach ($jobs as $key => $job) {
                ...
                $json[$key]['link'] = url('Pro/job/' . $job->slug);
                ...
                $json[$key]['project_level']['level_title'] = !empty($job->id) ? ProHelper::getProjectLevel($job->project_level) : '';
                $json[$key]['project_level']['level_sign'] = 0;
                $json[$key]['project_type'] = !empty($job->project_type) ? $job->project_type : '';
                $json[$key]['project_duration'] = !empty($job->duration) ? ProHelper::getJobDurationList($job->duration) : '';
                ...
                $json[$key]['skills'] = $skills_args;
                $json[$key]['employer_name'] = !empty($user_object->id) ? ProHelper::getUserName($user_object->id) : '';
    ...
    public function updateProfile(Request $request)
    {
        ...
        $role_id = ProHelper::getRoleByUserID($request['user_id']);
        $packages = DB::table('pro_items')->where('subscriber', $request['user_id'])->count();
        $package_options = ProPackage::select('options')->where('role_id', $role_id)->first();
        ...
        if ($packages > 0) {
            ...
            $location = ProLocation::find($request['location_id']);
            ...
            if ($request['department']) {
                $department = ProDepartment::find($request['department']);
    ...
    public function listings()
    {
        ...
        if (!empty($type)) {
            //Hourly Rates
            if ($type === 'rates') {
                $rates = ProHelper::getHourlyRate();
                if (!empty($rates)) {
                    $count = 0;
                    foreach ($rates as $value => $rate) {
                        $json[$count]['title'] = !empty($rate) ? ProHelper::getHourlyRate($value) : '';
            ...
            if ($type === 'english_levels') {
                $english_levels = ProHelper::getEnglishLevelList();
                if (!empty($english_levels)) {
                    $count = 0;
                    foreach ($english_levels as $value => $level) {
                        $json[$count]['title'] = !empty($level) ? ProHelper::getEnglishLevelList($value) : '';
            ...
            if ($type === 'freelancer_level') {
                $freelancer_level = ProHelper::getFreelancerLevelList();
                if (!empty($freelancer_level)) {
                    $count = 0;
                    foreach ($freelancer_level as $value => $level) {
                        $json[$count]['title'] = !empty($level) ? ProHelper::getFreelancerLevelList($value) : '';
                        $json[$count]['value'] = !empty($level) ? $value : '';
                        $count++;
                    }
                    return Response::json($json, 200);
            ...
            if ($type === 'duration_list') {
                $duration_list  = ProHelper::getJobDurationList();
                if (!empty($duration_list)) {
                    $count = 0;
                    foreach ($duration_list as $value => $list) {
                        $json[$count]['title'] = !empty($list) ? ProHelper::getJobDurationList($value) : '';
            ...
            if ($type === 'project_type') {
                $project_type  = ProHelper::getProjectType();
                if (!empty($project_type)) {
                    $count = 0;
                    foreach ($project_type as $value => $type) {
                        $json[$count]['title'] = !empty($type) ? ProHelper::getProjectType($value) : '';
            ...
            //Reason Types
            if ($type === 'reason_type') {
                $reason_types = ProHelper::getReportReasons();
            ...
            if ($type === 'project_level') {
                $project_level  = ProHelper::getProjectLevel();
                if (!empty($project_level)) {
                    $count = 0;
                    foreach ($project_level as $value => $level) {
                        $json[$count]['title'] = !empty($level) ? ProHelper::getProjectLevel($value) : '';
                        $json[$count]['value'] = !empty($level) ? $value : '';
                        $count++;
                    }
                    return Response::json($json, 200);
    ...
    public function getCategories()
    {
        ...
        if (!empty($categories) && $categories->count() > 0) {
            foreach ($categories as $key => $cat) {
                $cat_obj = ProCategory::find($cat->id);
                $json[$key]['image'] = !empty($cat->id) ? url('uploads/categories/' . $cat->image) : array();
                $json[$key]['link'] = url('Pro/search-results?type=job&category%5B%5D=' . $cat->id);
                ...
                if (!empty($email_settings) && !empty($email_settings[0]['email'])) {
                    ...
                    if ($request['type'] == 'job') {
                        ...
                        if (!empty($report_project_template->id)) {
                            ...
                            $email_params['link'] = url('Pro/job/' . $job->slug);
                            $email_params['report_by_link'] = url('Pro/profile/' . $user->slug);
                            $email_params['reported_by'] = ProHelper::getUserName($user->id);
    ...
    public function taxonomies()
    {
        ...
        if (!empty($type)) {
            ...
            if ($type === 'skills') {
                ...
                if (!empty($skills)) {
                    foreach ($skills as $key => $skill) {
                        $skill_obj = ProSkill::find($skill->id);
            ...
            if ($type === 'languages') {
                ...
                if (!empty($languages)) {
                    foreach ($languages as $key => $lang) {
                        $lang_obj = ProSkill::find($lang->id);
            ...
            // Locations
            if ($type === 'locations') {
                $locations = ProLocation::all();
                if (!empty($locations)) {
                    foreach ($locations as $key => $location) {
                        $location_obj = ProLocation::find($location->id);
            ...
            if ($type === 'no_of_employes') {
                $no_of_employes = ProHelper::getEmployeesList();
            ...
            if ($json['type'] == "success") {
                ...
                if (!empty($email_settings) && !empty($email_settings[0]['email'])) {
                    ...
                    if (!empty($send_freelancer_offer->id)) {
                        $job = ProJob::where('id', $request['job_id'])->first();
                        ...
                        $email_params['project_link'] = url('Pro/job/' . $job->slug);
                        $email_params['employer_profile'] = url('Pro/profile/' . $user->slug);
                        $email_params['emp_name'] = ProHelper::getUserName($request['user_id']);
    ...
    public function sendResetLinkEmail(Request $request)
    {
        ...
        $random_number = ProHelper::generateRandomCode(10);
        ...
        if (!empty($email_settings) && !empty($email_settings[0]['email'])) {
            ...
            if (!empty($template->id)) {
                ...
                $email_params['name'] = ProHelper::getUserName($user->id);
    ...
    public function postJob(Request $request)
    {
        ...
        $package_item = ProItem::where('subscriber', $current_user)->first();
        $package = ProPackage::find($package_item->product_id);
        ...
        $posted_jobs = ProJob::where('user_id', $current_user)->count();
        $posted_featured_jobs = ProJob::where('user_id', $current_user)->where('is_featured', 'true')->count();
        $size = !empty($request['size']) ? $request['size'] : '';
        if (!empty($package) && $current_date <= $expiry_date) {
            ...
            if ($posted_jobs >= intval($option['jobs'])) {
                ...
                $random_number = ProHelper::generateRandomCode(8);
                ...
                if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                    $job = ProJob::where('user_id', $current_user)->latest()->first();
                    $email_params = array();
                    ...
                    if (!empty($new_posted_job_template->id) || !empty(new_posted_job_template_employer)) {
                        ...
                        $email_params['posted_job_link'] = url('Pro/job/' . $job->slug);
                        $email_params['name'] = ProHelper::getUserName($current_user);
                        $email_params['link'] = url('Pro/profile/' . $user->slug);
