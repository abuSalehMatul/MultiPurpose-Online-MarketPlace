 ...
 *
 */
class ProJobController extends Controller
{
    /**
     ...
     * @return void
     */
    public function __construct(ProJob $job)
    {
        $this->job = $job;
    }
    ...
    public function postJob()
    {
        ...
        $project_levels = ProHelper::getProjectLevel();
        $job_duration = ProHelper::getJobDurationList();
        $freelancer_level = ProHelper::getFreelancerLevelList();
        $skills = ProSkill::pluck('title', 'id');
        $categories = ProCategory::pluck('title', 'id');
        ...
        return view(
            'pro.back-end.employer.jobs.create',
            compact(
                'english_levels',
                'languages',
                'project_levels',
    ...
    public function index()
    {
        ...
        return view('pro.back-end.employer.jobs.index', compact('job_details', 'symbol'));
    }
    ...
    {
        if (!empty($job_slug)) {
            $job = ProJob::where('slug', $job_slug)->first();
            $json = array();
            $languages = Language::pluck('title', 'id');
            $locations = ProLocation::pluck('title', 'id');
            $skills = ProSkill::pluck('title', 'id');
            ...
            $english_levels = ProHelper::getEnglishLevelList();
            $job_duration = ProHelper::getJobDurationList();
            $freelancer_level_list = ProHelper::getFreelancerLevelList();
            $attachments = !empty($job->attachments) ? unserialize($job->attachments) : '';
            if (!empty($job)) {
                return View(
                    'pro.back-end.employer.jobs.edit',
                    compact(
                        'job',
                        'project_levels',
                        'english_levels',
    ...
    public function getAttachmentSettings(Request $request)
    {
        $json = array();
        if ($request['slug']) {
            $settings = ProJob::where('slug', $request['slug'])
    ...
    public function store(Request $request)
    {
      ...
      //  return $json;
        ...
        $package_item = ProItem::where('subscriber', Auth::user()->id)->first();
        $package = !empty($package_item) ? ProPackage::find($package_item->product_id) : '';
        ...
        $posted_featured_jobs = ProJob::where('user_id', Auth::user()->id)->where('is_featured', 'true')->count();
        if ($package != '' && $current_date <= $expiry_date) {
            ...
            if ($posted_jobs >= intval($option['jobs'])) {
                ...
                if ($job_post = 'success') {
                    ...
                    return redirect( 'Pro/employer/dashboard/manage-jobs');
                }
            }
        } else {
            $json['type'] = 'error';
