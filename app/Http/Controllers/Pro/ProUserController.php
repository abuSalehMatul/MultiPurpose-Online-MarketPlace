...
class ProUserController extends Controller
{
     ...
     * @return void
     */
    public function __construct(User $user, ProProfile $profile)
    {
        $this->user = $user;
    ...
    public function accountSettings()
    {
        ...
        return view(
            'pro.pro.back-end.settings.security-settings',
    ...
    {
        return view('pro.back-end.settings.reset-password');
    }
                ...
                if ($request->confirm_password === $request->confirm_new_password) {
                    ...
                    if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                        ...
                        if (!empty($template->id)) {
                            ...
                            $email_params['name'] = ProHelper::getUserName($user_id);
    ...
    public function emailNotificationSettings()
    {
        ...
        return view('pro.back-end.settings.email-notifications', compact('user_email'));
    }
    ...
    {
        return view('pro.back-end.settings.delete-account');
    }
    ...
    public function destroy(Request $request)
    {
        ...
        if (Hash::check($request->old_password, $user->password)) {
            if (!empty($user_id)) {
                ...
                $user->delete();
                if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                    $delete_reason = ProHelper::getDeleteAccReason($request['delete_reason']);
                    $email_params = array();
    ...
    public function deleteUser(Request $request)
    {
        ...
        if (!empty($request['user_id'])) {
            ...
            if (!empty($user)) {
                ...
                DB::table('pro_offers')->where('user_id', $request['user_id'])
                    ->orWhere('freelancer_id', $request['user_id'])->delete();
                DB::table('messages')->where('user_id', $request['user_id'])
                    ->orWhere('receiver_id', $request['user_id'])->delete();
                DB::table('pro_items')->where('subscriber', $request['user_id'])
                    ->delete();
    ...
    public function getSavedItems(Request $request, $role = '')
    {
        if (Auth::user()) {
            ...
            $symbol            = !empty($currency) && !empty($currency[0]['currency']) ? ProHelper::currencyList($currency[0]['currency']) : array();
            if ($request->path() === 'employer/saved-items') {
                return view(
                    'pro.back-end.employer.saved-items',
                    compact(
                        'profile',
                        'saved_jobs',
                        'saved_freelancers',
                ...
                return view(
                    'pro.back-end.freelancer.saved-items',
                    compact(
                        'profile',
                        'saved_jobs',
                        'saved_freelancers',
    ...
    public function submitReview(Request $request)
    {
        ...
        if (Auth::user()) {
            ...
            if ($submit_review['type'] == "success") {
                ...
                // Send Email
                $freelancer = ProProposal::select('freelancer_id')->where('status', 'completed')->first();
                $user = User::find($freelancer->freelancer_id);
                $job = ProJob::find($request['job_id']);
                //send email
                if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                    $email_params = array();
                    ...
                    if (!empty($job_completed_template->id)) {
                        ...
                        $email_params['completed_project_link'] = url('Pro/job/' . $job->slug);
                        $email_params['name'] = ProHelper::getUserName($freelancer->freelancer_id);
    ...
    public function storeReport(Request $request)
    {
        ...
        if (Auth::user()) {
            ...
            $report = ProReport::submitReport($request);
            if ($report == 'success') {
                $json['type'] = 'success';
                $user = $this->user::find(Auth::user()->id);
                //send email
                if ($request['report_type'] == 'job-report'
                    ...
                    if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                        ...
                        if ($request['report_type'] == 'job-report') {
                            ...
                            if (!empty($report_project_template->id)) {
                                ...
                                $email_params['reported_by'] = ProHelper::getUserName(Auth::user()->id);
                            ...
                            if (!empty($report_employer_template->id)) {
                                ...
                                $email_params['reported_employer'] = ProHelper::getUserName($request['id']);
                                $email_params['link'] = url('Pro/profile/' . $employer->slug);;
                                $email_params['report_by_link'] = url('Pro/profile/' . $user->slug);
                                $email_params['reported_by'] = ProHelper::getUserName(Auth::user()->id);
                                $email_params['message'] = $request['description'];
                                Mail::to(config('mail.username'))
                            ...
                            if (!empty($report_freelancer_template->id)) {
                                ...
                                $email_params['reported_freelancer'] = ProHelper::getUserName($request['id']);
                    ...
                    if (!empty($freelancer_job_cancelled->id)) {
                        if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                            ...
                            $job = ProJob::find($request['id']);
                            $proposal = ProProposal::where('id', $request['proposal_id'])->first();
                            $freelancer = User::find($proposal->freelancer_id);
                            $email_params['project_title'] = $job->title;
                            $email_params['cancelled_project_link'] = url('Pro/job/' . $job->slug);
                            $email_params['name'] = ProHelper::getUserName($proposal->freelancer_id);
                            $email_params['link'] = url('Pro/profile/' . $freelancer->slug);
                            $email_params['employer_profile'] = url('Pro/profile/' . Auth::user()->slug);
                            $email_params['emp_name'] = ProHelper::getUserName(Auth::user()->id);
    ...
    public function sendPrivateMessage(Request $request)
    {
        if (Auth::user()) {
            ...
            if (!empty($request['description'])) {
                $user_id = Auth::user()->id;
                $json = array();
                $proposal = DB::table('pro_proposals')->select('status')->where('id', $request['proposal_id'])->get()->first();
    ...
    {
        if (!empty($id)) {
            $package_options = ProHelper::getPackageOptions(Auth::user()->getRoleNames()[0]);
            $package = Package::find($id);
            $payout_settings = ProSiteManagement::getMetaValue('commision');
            $payment_gateway = !empty($payout_settings) && !empty($payout_settings[0]['payment_method']) ? $payout_settings[0]['payment_method'] : array();
            $symbol = !empty($payout_settings) && !empty($currency[0]['currency']) ? ProHelper::currencyList($payout_settings[0]['currency']) : array();
            return view::make('pro.back-end.package.checkout', compact('package', 'package_options', 'payment_gateway', 'symbol'));
        }
    }
        ...
        if (Auth::user()->getRoleNames()[0] != 'admin' && Auth::user()->getRoleNames()[0] === 'employer') {
            ...
            if ($type === 'project') {
                $invoices = DB::table('invoices')
                    ...
                    ->get();
                return view('pro.back-end.employer.invoices.project', compact('invoices', 'type', 'expiry_date', 'symbol'));
            } elseif ($type === 'package') {
                $invoices = DB::table('invoices')
                    ->join('items', 'items.invoice_id', '=', 'invoices.id')
                    ...
                    ->where('invoices.type', $type)
                    ->get();
                return view('pro.back-end.employer.invoices.package', compact('invoices', 'type', 'expiry_date', 'symbol'));
            }
        }
    }
    ...
    public function getFreelancerInvoices($type = '')
    {
        if (Auth::user()->getRoleNames()[0] != 'admin' && Auth::user()->getRoleNames()[0] === 'freelancer') {
            ...
            $symbol = !empty($currency) && !empty($currency[0]['currency']) ? ProHelper::currencyList($currency[0]['currency']) : array();
            if ($type === 'project') {
                return view('pro.back-end.freelancer.invoices.project', compact('invoices', 'type', 'expiry_date', 'symbol'));
            } elseif ($type === 'package') {
                return view('pro.back-end.freelancer.invoices.package', compact('invoices', 'type', 'expiry_date', 'symbol'));
            }
        } else {
            abort(404);
        }
    ...
    public function showInvoice($id)
    {
        if (!empty($id)) {
            ...
            $code = ProHelper::currencyList($currency_code);
            $symbol = !empty($code) ? $code['symbol'] : '$';
            if (Auth::user()->getRoleNames()->first() === 'freelancer') {
                return view::make('pro.back-end.freelancer.invoices.show', compact('invoice_info', 'symbol', 'currency_code'));
            } elseif (Auth::user()->getRoleNames()->first() === 'employer') {
                return view::make('pro.back-end.employer.invoices.show', compact('invoice_info', 'symbol', 'currency_code'));
            }
        }
    }
    ...
    {
        $profile = ProProfile::where('user_id', Auth::user()->id)
            ->get()->first();
        ...
        return view(
            'pro.back-end.admin.profile-settings.personal-detail.index',
            compact(
                'banner',
                'avater',
                'tagline',
    ...
    {
        $path = ProHelper::PublicPath() . '/uploads/users/temp/';
        if (!empty($request['hidden_avater_image'])) {
            $profile_image = $request['hidden_avater_image'];
            return ProHelper::uploadTempImage($path, $profile_image);
                ...
                if ($storeProjectOffers == "success") {
                    ...
                    if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                        ...
                        if (!empty($send_freelancer_offer->id)) {
                            $job = ProJob::where('id', $request['offer']['project'])->first();
    ...
    {
        $job = ProJob::where('slug', $slug)->first();
        $reasons = ProHelper::getReportReasons();
        return View(
            'pro.back-end.freelancer.jobs.dispute',
            compact(
                'job',
                'reasons'
            )
        ...
        if ($storeDispute == "success") {
            ...
            if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
                ...
                if (!empty($dispute_raised_template->id)) {
                    ...
                    $email_params['project_title'] = $job->title;
                    $email_params['project_link'] = url('Pro/job/' . $job->slug);
                    $email_params['sender_link'] = url('Pro/profile/' . $user->slug);
                    $email_params['name'] = ProHelper::getUserName(Auth::user()->id);
                    $email_params['msg'] = $request['description'];
    ...
    public function userListing()
    {
        if (Auth::user() && Auth::user()->getRoleNames()->first() === 'admin') {
            ...
            return view('pro.back-end.admin.users.index', compact('users'));
        } else {
            abort(404);
        }
    }
    ...
    {
        if (!empty(config('mail.username')) && !empty(config('mail.password'))) {
            $random_number = ProHelper::generateRandomCode(4);
            $verification_code = strtoupper($random_number);
            $user = User::find(Auth::user()->id);
            $user->verification_code = $verification_code;
            $user->save();
            $register_form = ProSiteManagement::getMetaValue('reg_form_settings');
            ...
            return View('pro.back-end.email-verify.index', compact('reg_three_title', 'reg_three_subtitle', 'register_image'));
        } else {
    ...
    public function verifyUserEmailCode(Request $request)
    {
        ...
        if (!empty($request['code'])) {
            if ($request['code'] === $user->verification_code) {
                ...
                $user->save();
                return Redirect::to('Pro/'.$role.'/dashboard');
            } else {
                Session::flash('error', trans('lang.ph_email_warning'));
                return Redirect::back();
            }
    ...
    public function activate_pro($id){
        $id=base64_decode($id);
        $user=User::find($id);
        $role_id = Role::where('name', 'pro')->select('id')->first();
        $role = DB::table('model_has_roles')->where('model_id', $user->id)->where('role_id', $role_id->id)->first();
        if (!$role) {
            DB::table('model_has_roles')->insert([

                ['role_id' => $role_id->id, 'model_type' => 'App\User', 'model_id' => $user->id]
