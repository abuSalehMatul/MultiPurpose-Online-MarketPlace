 ...
 *
 */
class ProPaypalController extends Controller
{
        ...
        if (Auth::user()) {
            ...
            if (Auth::user()->getRoleNames()[0] == "employer") {
                if ($product_type == 'project') {
                    return Redirect::to('Pro/employer/jobs/hired');
                } else {
                    return Redirect::to('Pro/dashboard/packages/employer');
                }
            } else if (Auth::user()->getRoleNames()[0] == "freelancer") {
                session()->forget('type');
                return Redirect::to('Pro/dashboard/packages/freelancer');
            }
        } else {
            abort(404);
        }
    ...
    public function getExpressCheckoutSuccess(Request $request)
    {
        if (Auth::user()) {
            ...
            if (in_array(strtoupper($response['ACK']), ['SUCCESS', 'SUCCESSWITHWARNING'])) {
                ...
                return redirect('Pro/paypal/redirect-url');
            } else {
                abort(404);
            }
        }
    ...
    protected function getCheckoutData($recurring, $success)
    {
        if (Auth::user()) {
            ...
            if ($recurring === true) {
                ...
                $data['return_url'] = url('Pro/paypal/ec-checkout-success?mode=recurring');
                ...
                $data['return_url'] = url('Pro/paypal/ec-checkout-success');
            }
    ...
    protected function createInvoice($cart, $status, $payment_detail)
    {
        ...
        collect(
            ...
            function ($product) use ($invoice) {
                ...
                if ($product_type == 'package') {
                    $item = DB::table('pro_items')->select('id')->where('subscriber', $product['subscriber_id'])->first();
                    if (!empty($item)) {
        ...
        if (Auth::user()) {
            if ($product_type == 'package') {
                if (session()->has('product_id')) {
                    $package_item = \App\ProModel\ProItem::where('subscriber', Auth::user()->id)->first();
                    $id = session()->get('product_id');
            ...
            if ($product_type == 'project') {
                ...
                if ($role === 'employer') {
                    if (!empty($user->email)) {
                        ...
                        if (!empty($template->id)) {
                            $template_data = EmailTemplate::getEmailTemplateByID($template->id);
                            $email_params['employer'] = ProHelper::getUserName(Auth::user()->id);
