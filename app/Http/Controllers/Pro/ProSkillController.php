...
class ProSkillController extends Controller
{
     ...
     * @return void
     */
    public function __construct(ProSkill $skill)
    {
        $this->skill = $skill;
    }
    ...
    public function index(Request $request)
    {
        ...
        return View::make(
            'pro.back-end.admin.skills.index',
            compact('skills')
        );
    }
    ...
    public function edit($id)
    {
        if (!empty($id)) {
            ...
            if (!empty($skills)) {
                return View::make(
                    'pro.back-end.admin.skills.edit',
                    compact('id', 'skills')
                );
                return Redirect::to('Pro/admin/skills');
            }
        }
    }
    ...
    {
        $json = array();
        $db_skills = ProSkill::select('id')->get()->pluck('id')->toArray();
        $freelancer_skills = ProSkill::getFreelancerSkill(Auth::user()->id);
        $result = array_diff($db_skills, $freelancer_skills);
        if (!empty($result)) {
            $skills = DB::table('pro_skills')
                ->whereIn('id', $result)
                ->orderBy('title')->get()->toArray();
        } else {
            $skills = ProSkill::select('title', 'id')->get()->toArray();
        }
        if (!empty($skills)) {
            $json['type'] = 'success';
            $json['skills'] = $skills;
    ...
    public function getJobSkills(Request $request)
    {
        ...
        if (!empty($request['slug']) && $request['slug'] != "post-job") {
            $job = ProJob::where('slug', $request['slug'])->select('id')->first();
            $db_skills = ProSkill::select('id')->get()->pluck('id')->toArray();
            $job_skills = ProSkill::getJobSkill($job->id);
            if (!empty($job_skills)) {
